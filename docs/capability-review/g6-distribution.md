...
```aontu
# consumer.aontu — what a user wants to write today, and cannot
...
repositories each copied `service.aontu` in January; one was edited in
March:

```aontu
# repo-a/vendor/service.aontu  (as reviewed)
service: close({ name: string, port: *8080 | integer })
```

```aontu
# repo-b/vendor/service.aontu  (edited later; nothing objects)
...
```aontu
# service.aontu, before a tidy-up commit
service: { port: *8080 | integer, name: string }
```

```aontu
# service.aontu, after a tidy-up — new bytes, identical meaning
# The service schema.
...
- **Source loading.** `@"path"` loads, parses, and unifies another
  ...
  Extensions `.aontu` are tried for bare paths; relative
...
**B. Piggyback on host package managers.** Ship `.aontu` files inside
...
[ADR-039](../../ADR.md#adr-039--the-package-system-has-one-vocabulary-one-set-of-files-and-three-pins).
Where this document says `mod.aon`, read `pkg.aontu`; where it says
`aontu_meta/mod-lock.aon`, read `aontu_meta/pkg-lock.aontu`; where a
