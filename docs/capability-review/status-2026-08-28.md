...
0.53.0 on each) and both are left open: a `k:v` pair written *before* a
`&:` spread makes TypeScript drop the spread and generate where Go
refuses, and a conjunct of unequal-length lists paths its finding at the
list in one port and at the element in the other.
