...
positions converting — `use-cases/01-service-catalog/spec.aon:74`
...
deletion touched no `.aon` that generates anything.
...
render\` from model.aon`, which named a verb that no longer exists.
...
is a `pkg.aon` field, read and refused in `ts/src/mod.ts` and
