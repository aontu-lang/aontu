...
| **6** — `walk`, the manifest, and the verb | M | **NOT STARTED** | One run over N outputs. **The name is unsettled and is decided before phase 4**: this row says `aontu gen` with a `std/gen` manifest, the design says `aontu render` with the manifest under `@"std/code"`, and it names a verb, an MCP tool, a file in each port, help text the suite asserts identical across the builds, and every transcript in the reference. |
