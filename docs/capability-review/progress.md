...
`docs/reference-functions.md`, "Subsumption" — the per-atom rules for
...
sentence survives in `docs/reference-functions.md`'s constraint section
...
`docs/reference-functions.md` "Named constraint aliases" and as a task in
...
[Grammars: `abnf()` and `parse()`](../reference-functions.md#grammars-abnf-and-parse).
...
`MODULE_RE` (`ts/src/mod.ts`) and `moduleRe` (`go/mod.go`) are what
admit the suffix in a path; `majorOf` (`ts/src/mod-tool.ts`,
`go/modtool.go`) is what derives it from `mod.version` for the
...
the computed check replaces. In the
shared suite, 17 of `test/spec/mod.tsv`'s 32 rows carry `@1` (25
occurrences, and two more in the file's comments), and twelve fixture
directories under `test/spec/files/` are named for it. The rename
story below is still phase 3 work; what is wrong above is the claim
that the naming settlement costs no code.
