...
[`docs/contributing/conventions.md`](../contributing/conventions.md#known-tsgo-divergences),
recording why the divergence record moved to
...
5. **Counts of the shared suite live here and nowhere else.** All eight
   ...
   **113 `.tsv` files, 111 row-bearing, 5,166 rows**, in twenty-nine
   modes — `errc` 987, `gens` 970, `canon` 935, `gen` 628, `err` 319,
   `fmt` 176, `view` 174, `errcode` 168, `vet` 128, `subsume` 114,
   `query` 93, `jsonschema` 57, `why` 53, `hcanon` 42, `patch` 41,
   `graph` 38, `views` 37, `hash` 34, `template` 34, `diff` 28,
   `fmt-lint` 28, `relation` 25, `reaches` 19, `trim` 11, `trace` 9,
   `agentsmd` 7, `fmt-template` 7, `fmt-template-lint` 3,
   `fmt-refuse` 1.
   (Re-derived 2026-09-18, twice: the first derivation that day read
   5,153 and was overtaken within hours by `cf75be16`, which added
   thirteen `alias.tsv` rows — which is the argument for keeping the
   reproduction commands here rather than the number anywhere else.
   **A MODE LEFT THE LIST, which a count alone would have hidden**:
   the `render` mode and its 106 rows are gone,
...
G4.1 (ADR-014), G8.4 (ADR-018), G9.4 (ADR-038, the renderer and its
lowering). Three were retired before any of them was built — G10.5 (ADR-019, whose log service "will not be built"),
...
- **Phase A — make the claim true.** Nearly done. The trust contract
  ...
  the `vet` rows both runners execute — counted by rule 5 above, not
  here, because the figure this sentence carried (46, at G2.6's
  landing) had nearly trebled by the time anybody re-read it. G1 landed
  more than the sequencing table's "constraint algebra core (bounds,
  regex, length/count)" asked for: cross-field residuation and the `must`
...
- **Phase B — differentiate.** Underway, and **G3 is complete** —
  ...
  (G3.1–2), its rules and the `subsume` rows of
  `test/spec/subsume.tsv` (G3.0, 97 at landing), the `subsume` and
  ...
  **canon-hash pins meaning rather than text** (G6.0–1): the hash form
  ...
  `aontu model get` selects one node by path and renders it plainly or
  as a lattice abstraction, in both ports, with the "view subsumes
  truth" property mechanically asserted by every projection row rather
  than promised. **Provenance is a verb** (G7.3–4): `aontu model why`
  names every contribution to a value with the site it was written at,
  from a recorder that hooks the one place every meet passes through
  and is off by default. And **patch is a verb** (G7.5):
  `aontu model set` appends
...
go 0.1.16 on 2026-09-06 (`47d36b8`, the merge of #169), 0.59.0 /
go 0.1.17 on 2026-09-07 (`64b8f84`, #173 squashed onto `main`), 0.60.0
...
boundary list; the catalog's own schema read
`dependsOn?: rel($.std.Service) & re("^svc_") & acyclic() &
inverse(dependedOnBy)` that day, with plain-list data and patch
positions converting — `use-cases/01-service-catalog/spec.aontu:74`
spells the same declaration `rel($.aontu.System.Service) &
%CatalogAddr & acyclic() & inverse(dependedOnBy)` today, the bundled
vocabulary having been renamed by ADR-028 and the pattern lifted into
an alias). The `relations:` magic key is GONE — ADR-010's
...
`x: {%a = 1}` refused, `a: @"f.aontu"` refused (left writable, the
includer's `%b` is what `f.aontu`'s own `%b` would reach), and `@"f.aontu"`
...
by `bare_punct` (class parse, since 0.58.0): a bare string holds
letters, digits, `-` and `_` and nothing else, so `foo = 1` and
`a: x=y` are `test/spec/alias.tsv:129` and `:130`, both `errc`. The
...
source can write — and `{ %a } = @"f.aontu"` is lexed as one head token
...
`$.%a@#1` in Go and `$.%a@/abs/path/to/the/file.aontu` in TypeScript --
...
well-formed wrong config" — was unchanged: `port: >=1024` evaluated to
`{"port":">=1024"}` and exited 0, in both ports. Choosing function
syntax removed the *need* to reuse `>` without changing what `>`
did, so the capability landed while the defect that motivated
...
**Reversed again by `bare_punct`, 0.58.0, and this is where the story
ends.** The refusal §45 proposed did land, on a narrower and better
argument than §45's: not that `>10` is a wrong config, but that a bare
string holds letters, digits, `-` and `_` and every other punctuation
character is syntax or an error, never silently part of a string. So
`port: >=1024` now exits 1 in both ports with `[aontu/bare_punct]`,
and the error's own hint carries `a: >10 -> nil  # Not an operator:
write above(10)`. The 2026-08-28 retraction was right about §45's
reasoning and wrong about the outcome, and a reader who stopped at the
paragraph above would have run the old behaviour expecting it to hold.
...
2. **Relations were read from the `relations` key of the document
   root.** The design shows them there and never says how the pass
   finds them, so that became the rule, as the VOCABULARY's convention
   rather than the engine's. **Discharged by RELATIONS P2
   (2026-08-29).** The magic key is gone: `relations:` is ordinary
   user data, `declaredRelations()` and `meets()` are defined nowhere
   in either port, and the atoms register during unification at the
   field whose key is the predicate. `test/spec/relation.tsv:93`
   (`relations-key-is-user-space`) and `:94`
   (`relations-key-generates-as-data`) pin it from both sides.
...
1. **The NETWORK HALF was not in this build, and the CLI said so.**
   *(Discharged 2026-09-16: both verbs are built, as `aontu get` and
   `aontu publish` behind a total `ModuleFetch` transport — G10 phases
   3 and 4 under ADR-039. Nothing in either port names a missing half
   any more, so the rest of this item is the record of what G6.3
   shipped, not of what a reader will meet.)*
...
**Departures 2 and 3 are marked, 2026-09-13, by
[ADR-022](../../ADR.md#adr-022--compatibility-is-computed-so-the-major-leaves-the-name).**
Both are argued from the major sitting in the name, and that entry
takes it out. The `.major` annotation key records the major an import
spells, and an import will spell none; departure 3's escape, that a
module which means to break bumps its major, is the route the entry
replaces with a computed three-part check and a new name the publisher
chooses. Neither departure is withdrawn: they record what shipped.
What is superseded is the reasoning they rest on, and because the
annotation keys are a published wire format, retiring `.major` is a
compatibility decision rather than an edit. **That decision was made
three days later, by removing its subject**: ADR-039 replaced the OCI
artifact with the specification's signed manifest, so there are no
annotation keys to retire — `com.github.rjrodger.aontu.canon` and
`.major` are written nowhere in either port.

Both remaining design items beyond this were network: `mod get` and
`mod publish` themselves (see the G6.3 departures). **Neither remains.**
They landed on 2026-09-16 as `aontu get` and `aontu publish`, and the
`mod` verb tier they were to be spelled under is gone: ADR-039's tiers
put the package operations under `aontu pkg <op>` with the six
common ones at the top level.
...
document. Then `aontu model set --in-place` used them.
...
`aontu vet2 schema.aontu good.json` printed `good.json` and exited 0,
...
the same date): every `aontu`/`aon` example in the page list the test
carries must parse, and every one that states its result — an `aontu`
fence immediately followed by a `json` fence — must generate exactly
that. The list at landing was `index.md`, `tutorial.md`, `how-to.md`
and `reference-language.md`; it has since grown the tutorial set,
the reference supplements and every guide under `docs/how-to/`, and
`how-to.md` no longer exists — the test keeps it only behind an
`existsSync` guard, which is why the split did not have to touch the
gate. The page list lives in the test, not here. The skill
...
below is LANDED or RETIRED but phase 0, whose one remaining defect is
recorded against it. (Three remained when this line was written; §60
and §63 were fixed on 2026-09-07 and the row below says so. §57, a
recursive spread conjoined with a map, is the one left.)
...
and a human UI over the same data, generated from one `model.aontu` by
...
identically across the ports — `parser` 0.9.0, `abnf` 0.4.7 and `bnf`
0.1.10 at this landing, TypeScript having been a minor behind Go at
0.8.7, and the two `package.json`/`go.mod` pairs have moved together
since; read them rather than this line for today's versions. Exactly
rather than by range, and that rule is the durable part: `bnf` 0.1.11
peer-asked for `parser` 0.9.1, and 0.9.1 regressed `path($.z.x.a)` in
TypeScript alone, so a range would have taken a one-port regression
without a commit. Docs:
...
`xf-order.aontu` rendered ONE declaration list as both TypeScript and Go,
...
§12a said it would cost. `xf-domain-cmp.aontu` is removed: it was P1's
...
apart from what it protects.

**The split first, because `loadProfiles` is shared by three verbs.**
`aontu:render`'s `template` block is what `template --profile` and `fmt
--profile` vet against, and a P5 that began by deleting `render.ts`
would have taken two published verbs with it. `aontu:profile`,
`aontu:lang/text` and `aontu:lang/markdown` landed first, with
`ts/src/profile.ts`, `go/profile.go`, `go/cmd/aontu/profile.go` and
fourteen rows; `render` kept its own loader and its own vocabulary
until the commit that deleted both, taking the reader's loader as an
argument that went with it. **The name is the record's, not an
invention**: `aontu:profile` and `aontu:lang/<name>` are what these
carried before they were renamed under the verb that read them, and
ADR-035's own prose still spells them that way. The rename's reason,
that both should sit under that verb, is exactly what stopped being
true.

**Then the deletion.** Both renderers, both lowerings, the declaration
function, the two lowering profiles, `aontu:code` in both copies, four
spec files, the `render` verb in both CLIs and the `render` MCP tool.
The corpus needed nothing: P4 had already moved every generator, so the
deletion touched no `.aontu` that generates anything.

**Three things the deletion found.** (1) **`aontu trace` could not read
a template entry.** `render` read a generator written in the target's
own syntax directly; the new verb did not, so `aontu trace handler.ts`
was a parse error where `render --trace` had worked. `trace` now takes
`--marker` and `--profile` and desugars by extension, as `template` and
`fmt` do. (2) **`TraceEntry`'s JSON tags were not lexicographic**, which
`TestReportJSONTagsAreLexicographic` exists to catch and did not,
because the test names its types by hand and P6 never added the new one.
Both ports reordered, both still byte-identical. (3) **The emit mark's
inheritance through a meet was DEAD CODE in both ports.** `unite` and
`riders` each copied a mark from either operand when the result had
none; instrumenting every spec row shows the assignment never fires,
because unification answers a value that already carries it. Removed
from both, and the corpus's trace checks — two hundred and fifty
entries across twelve services, entry for entry in both ports — are
unchanged.

**What survives of the lowering, and why.** `nom` reads the casing, so
`splitWords`, `lowerASCII`, `upperASCII`, `capitalise` and `caseName`
move to `ts/src/casing.ts` and `go/casing.go` rather than dying with
`lower.ts`. Two arms went with the move: `caseName`'s `as-is` style and
its empty-word guard were the lowering's, and `nom` reaches neither.
The relative-specifier helper did NOT survive: §3a left it open until
P4 showed whether a migrated generator wanted one, and none does.

**The generated corpus said `aontu render` in its own banners.** Nine
generators and sixteen committed files carried `Generated by \`aontu
render\` from model.aontu`, which named a verb that no longer exists.
...
the `aontu/code/code.aontu` layout in `AGENTS.md`, and the whole of
...
**THE VERB RETURNED, 2026-09-16 ([ADR-040](../../ADR.md#adr-040--aontu-render-writes-the-component-tree-through-jostraca-in-both-ports)).** P5 left the writer to a pipe and a script, and jostraca ships no command of its own, so a model of one value could not become one text file without a Node script beside it, and a runtime installed by hand for the script to find. `aontu render` is back in both CLIs, writing through jostraca rather than through anything of aontu's: the row above records what landed.
...
- **`g10-transparency.md` phase 6 planned witnesses for a log the
  ...
  ground, and phase 6 was not re-read when it was. What survives is
  the constraint on the client: a checkpoint aontu verifies has to be
  cosignable from the first leaf, which the phase already says and
  which is not retrofittable. What does not survive is the project
  recruiting witnesses, gossiping checkpoints or mirroring its own
  log.

**The three P0 items get their owners here.** The 2026-09-05 status
document found the first only as a departure note, the second only
inside a NOT STARTED row, and the third nowhere at all. Publishing the
transparency client is now a named missing deliverable of G10 phase 2,
which is PARTIAL on that ground rather than carrying it as a departure
note inside a LANDED row. Closing the cache-identity hole stays inside
G10 phase 3, where it is a stated precondition of anything writing the
cache, and that is the right place for it. The file manifest has no
row in this register and gets none here: it is a change to the lock
entry rather than a phase, and its row follows the ADR that ratifies
the entry's shape.

Before those, the list this section carried was empty. All ten of the
earlier entries had been applied — four (the "design proposal"
headers, `docs/trust.md`'s depth-budget self-contradiction,
`AGENTS.md` on the divergence ledger, and `docs/test-coverage.md`'s
per-file table) in commit `94b63f9`, which is also the commit that
wrote the list; the remaining six with the G1 phase 4–5 landing.
What changed, for a reader tracing the history:
...
- All eight plan preambles that restated a row count now link protocol
  rule 5 above instead; none of the eight had a current one. The other
  three gap documents name no count, and G10's preamble gained the
  link on 2026-09-18, so all eleven now point at one place.
...
- The drifted `makeModelResolver` "~line 750" citations now name the
  file without a line number, which is the form that cannot drift
  again. This entry said G6 and G7 and was itself wrong twice over:
  G7's citation was clean, G6's still carried "~757–816", and G5
  carried "~line 757" unmentioned. Those two were dropped on
  2026-09-18, and the symbol is at `ts/src/lang.ts:1574` today, which
...
**Phases 1, 3 and 4 have landed; phase 2 is partial (its row says what is missing); phase 6 is NOT STARTED; phase 5 was retired on 2026-09-04.** Phases 3 and 4 landed on 2026-09-16 under ADR-039, both ports; their rows below record what landed and the three departures. The format
...
**Corrected 2026-09-13.** The paragraph above is true of ADR-020 and
false of [ADR-022](../../ADR.md#adr-022--compatibility-is-computed-so-the-major-leaves-the-name),
accepted the same day, which takes the major out of the name
altogether. That is language work in both ports and it moves rows.
`MODULE_RE` (`ts/src/mod.ts`) and `moduleRe` (`go/mod.go`) were what
admitted the suffix in a path; `majorOf` (`ts/src/mod-tool.ts`,
`go/modtool.go`) was what derived it from `mod.version` for the
manifest annotation and for the gate that skips the breaking check
when the prior version sits at another major, which is the behaviour
the computed check replaces. In the shared suite of that day, 17 of
`test/spec/mod.tsv`'s 32 rows carried `@1` (25 occurrences, and two
more in the file's comments), and twelve fixture directories under
`test/spec/files/` were named for it. The rename story below was still
phase 3 work then; what is wrong above is the claim that the naming
settlement costs no code.

**And it was done, so this paragraph is history too.** ADR-022 part 1
is built: the two regexes exclude `@` from a path element, so
`MODULE_RE` no longer matches `github.com/alice/widgets@1` and
`mod.tsv` carries one `@1` where it carried twenty-five —
`mod-major-is-not-a-module` at line 44, whose whole job is that the
suffix names nothing. No fixture directory under `test/spec/files/`
has an `@` in its name. `majorOf` and `MajorOf` are defined nowhere,
and their two source files were deleted by `cc240ec0` when ADR-039
replaced the OCI artifact that wanted the annotation; the gate's
major-skip went with the computed three-component check the entry
above proposed.
...
nothing had landed. **It has since: 2026-09-16, both ports.** `moved`
is a `pkg.aontu` field, read and refused in `ts/src/mod.ts` and
`go/mod.go` on the local path and in `ts/src/pkg-net.ts` and
`go/pkgnet.go` on the fetch path; `module_moved` is registered class
parse with its own hint in both ports; and `test/spec/mod.tsv:68`
(`mod-moved-refuses`) and `:69`
(`mod-moved-message`) are the shared rows this paragraph predicted,
against the `modmoved` fixture. It landed with G10 phase 3's verbs, as
this paragraph said it would.
...
| **6** — witnesses and hardening | M | NOT STARTED | **Rewritten 2026-09-16 for the federated design.** The log is Sigstore's, so its cosignatures, checkpoint gossip and witnesses are theirs to run and phase 2's client to verify; nothing here builds a witness. What remains this project's: mutation alerting on the repository (the client's `seen` records already refuse a version that disappears from the list, `list_rollback`, landed with phase 3; an auditor that replays the version lists and the transparency log against each other is not built), the git mirror of the read-path layout for availability and public replicability, and the hardening a first third-party publisher forces: rate and size limits on the write path, `subject_changed` and `runner_changed` surfaced to consumers, the escape hatch for an intended outcome change ADR-022 left open. Last deliberately: at zero third-party publishers every item here is cost with no incident to answer. |
...
ruled it for one verb: "`--check` is REQUIRED — `aontu trim f.aontu`
...
up any code a report hands it, is told when `view` or `relations` did
nothing rather than being answered silently (`render` was told too
until ADR-038 deleted the verb, and ADR-040's rebuilt one has no
vacuity signal — see the phase 4 row), can start from a working
document instead of inventing one (`aontu init`), reads
