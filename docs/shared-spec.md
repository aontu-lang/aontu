...
| `trace` | `traceRun(src)` (the PROVENANCE of a component tree: every piece a rule stamped under `$.out`) must produce the entries `expect` describes, in walk order, each carrying the `file` it reached, the `at` address it sits at, the model `node` the dispatch matched and the `rule` that wrote it. Only a rule's own pieces are stamped, so a line the document wrote by hand has no entry |
