...
Two verbs help before the language does. `aontu init <dir>` writes a
model, an instance of it, and a `check.sh` that asks the four
questions, which is a faster start than an empty file; `aontu explain
<code>` prints the long form of a refusal, so a code in a message
becomes a paragraph with examples.
...
engine you just used. When the document is ready to leave your
repository, [share a model as a package](tutorial-package.md) pins it
for whoever imports it, and [from a model to a file
tree](tutorial-generate.md) computes source files from it. The
