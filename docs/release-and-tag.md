...
| `nix run github:aontu-lang/aontu` | nothing: `flake.nix` builds from source | `vendorHash` in `flake.nix` updated when `go.mod` or `go.sum` change. `TestFlakeVendorHashMatchesTheModuleTree` in `go/flake_test.go` recomputes it and fails with the value to paste; `nix build` names it too |
