...
Generated files carry a banner naming aontu and `model.aontu`. The byte
gate compares them with the generator output and reports manual changes
as drift. It does not manage the handwritten files.
...
├── model.aontu     Handwritten service, entity, field, and action definitions
...
[`model.aontu`](model.aontu) is the whole input. It states the service,
...
| `views.aontu` | the four ERB pages, in canonical aontu |

`views.aontu` is the exception, for a reason in ERB rather than here:
...
Nothing in this moves a line of the generated app: the byte gate
...
One model, nine generators, one tree. The byte gate compares what
...
flowchart LR
  ...
  model["model.aontu<br/>service · 2 entities · 2 actions<br/>error envelope · seed data"]
  ...
  subgraph gen["gen/: nine generators"]
    ...
    views["views.aontu<br/>the four ERB pages"]
  end
  model --> gen
...
|---|---|---|
| a **fact about the system**: a field, an entity, a route shape, an action's rules, a seed row | `model.aontu` | the model is the only statement of it; every target follows |
...
and the byte gate compares the whole tree against what the
...
a diff to `model.aontu` or to `gen/`, because that is what caused it, and
