...
const pkg_net_1 = require("../dist/pkg-net");
const pkg_zip_1 = require("../dist/pkg-zip");
const pkg_1 = require("../dist/pkg");
const mod_1 = require("../dist/mod");
const SERVICE = 'name: string\nport: *8080 | integer\n';
const KEY_PEM = (0, node_crypto_1.generateKeyPairSync)('ed25519').privateKey
    .export({ format: 'pem', type: 'pkcs8' });
const OTHER_PEM = (0, node_crypto_1.generateKeyPairSync)('ed25519').privateKey
    .export({ format: 'pem', type: 'pkcs8' });
const KEY_ID = (0, pkg_net_1.keyIdFromPem)(KEY_PEM);
...
function servers(http, onServe, io) {
    return {
        lsp: () => undefined,
        mcp: () => undefined,
...
async function run(w, http, verb, args, onServe) {
    ...
    try {
        code = 'pkg' === verb ?
            await (0, cli_1.runPkg)(args, servers(http, onServe, io)) :
            await (0, cli_1.runPackageVerb)(verb, args, servers(http, onServe, io));
    }
    finally {
...
function publisher(w, name, version, src, extra = '') {
    const dir = Path.join(w.dir, name + '-' + version);
    write(dir, {
        'pkg.aontu': 'pkg: {path: "corp.example/' + name + '", version: "' + version +
            '", main: "main.aontu"}\n' + extra,
        'main.aontu': src,
    });
    return dir;
}
async function publish(w, tree, extra = []) {
    return run(w, (0, pkg_net_1.dirHttp)(w.repo), 'publish', ['--yes', '--key', w.key, '--to', w.repo, ...extra, tree]);
}
const REPO_BLOCK = 'repo: {base: ["' + FAKE + '"], trust: {"corp.example/*": {signer: "' +
    KEY_ID + '", inclusion: none}}}\n';
let consumers = 0;
function consumer(w, deps, repo = REPO_BLOCK) {
    const app = Path.join(w.dir, 'app' + (consumers++));
    write(app, {
        'pkg.aontu': 'pkg: {path: "corp.example/app"}\ndep: {' + deps + '}\n' + repo,
        'main.aontu': 'svc: @"corp.example/service"\nsvc: name: "auth"\n',
    });
    return app;
}
...
async function publisherWith(w, name, version, src, deps, extra = '') {
    const dir = publisher(w, name, version, src, 'dep: {' + deps + '}\n' + REPO_BLOCK + extra);
    const r = await run(w, (0, pkg_net_1.dirHttp)(w.repo), 'sync', [dir]);
    Assert.equal(r.code, 0, r.out + r.err);
    return dir;
}
const at = (w, name) => Path.join(w.repo, 'pkg', 'corp.example', name, '@v');
...
function backdate(w, name, versions) {
...
function resign(w, name, version, edit, pem = KEY_PEM) {
    const file = Path.join(at(w, name), version + '.manifest');
    const m = readJson(file);
    edit(m);
    const bytes = Buffer.from(JSON.stringify(m) + '\n');
    Fs.writeFileSync(file, bytes);
    Fs.writeFileSync(Path.join(at(w, name), version + '.sig'), JSON.stringify((0, pkg_net_1.signDigest)(pem, (0, pkg_zip_1.sha256Hex)(new Uint8Array(bytes)))) + '\n');
}
function lockOf(app) {
    return Fs.readFileSync(Path.join(app, 'aontu_meta', 'pkg-lock.aontu'), 'utf8');
}
(0, node_test_1.describe)('pkg-net', () => {
    (0, node_test_1.test)('publish-then-sync-round-trip', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        ...
        Assert.deepEqual(Fs.readdirSync(at(w, 'service')).sort(), ['1.4.2.manifest', '1.4.2.sig', '1.4.2.zip', 'list']);
        Assert.ok(Fs.existsSync(Path.join(w.repo, 'pkg', 'corp.example', 'service', '@latest')));
        Assert.ok(Fs.existsSync(Path.join(w.repo, 'advisory', 'corp.example', 'service.aontu')));
        ...
        const lock = (0, pkg_1.readLock)(app)['corp.example/service'];
        Assert.equal(lock.v, '1.4.2');
        Assert.match(lock.canon, /^aon1-/);
        Assert.match(lock.archive, /^sha256:/);
        Assert.match(lock.manifest, /^sha256:/);
        const vendored = Path.join(app, 'aontu_meta', 'vendor', 'corp.example', 'service');
        Assert.ok(Fs.existsSync(Path.join(vendored, 'aontu_meta', 'manifest.aontu')));
        Assert.ok(Fs.existsSync(Path.join(vendored, 'aontu_meta', 'proof.aontu')));
        Assert.ok(!Fs.existsSync(Path.join(vendored, 'aontu_meta', 'pkg-lock.aontu')));
        Assert.ok(Fs.existsSync(Path.join(w.cache, 'aontu', 'pkg', 'download', 'corp.example', 'service', '@v', '1.4.2.zip')));
        Assert.ok(Fs.existsSync(Path.join(w.cache, 'aontu', 'pkg', 'seen', 'corp.example', 'service', '1.4.2.aontu')));
        Assert.ok(Fs.existsSync(Path.join(w.cache, 'aontu', 'pkg', 'store', lock.canon, 'corp.example', 'service', 'main.aontu')));
    ...
    (0, node_test_1.test)('frozen-refuses-a-lock-that-would-change', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        ...
        Fs.appendFileSync(Path.join(app, 'pkg.aontu'), 'dep: "corp.example/common": {v: "1.0.0"}\n');
        ...
        Assert.ok(!Fs.existsSync(Path.join(w.cache, 'aontu', 'pkg', 'download', 'corp.example', 'common')), 'frozen fetched before refusing');
        Assert.equal((await run(w, http, 'sync', [app])).code, 0);
        // A tampered vendor tree repins; a dependency dropped from the
        // package file drops from the lock. Both are changes.
        const main = Path.join(app, 'aontu_meta', 'vendor', 'corp.example', 'service', 'main.aontu');
        ...
        Fs.writeFileSync(Path.join(app, 'pkg.aontu'), Fs.readFileSync(Path.join(app, 'pkg.aontu'), 'utf8').split('\n')
    ...
    (0, node_test_1.test)('sync-replaces-a-vendor-tree-at-another-version', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
        const app = consumer(w, '"corp.example/service": {v: "1.4.2"}');
        write(Path.join(app, 'aontu_meta', 'vendor', 'corp.example', 'service'), {
            'pkg.aontu': 'pkg: {path: "corp.example/service", version: "1.4.1", main: "main.aontu"}\n',
            'main.aontu': 'name: string\n',
        });
        const r = await run(w, http, 'sync', [app]);
        Assert.equal(r.code, 0, r.out);
        Assert.match(r.out, /fetched: corp.example\/service 1.4.2/);
        Assert.match(Fs.readFileSync(Path.join(app, 'aontu_meta', 'vendor', 'corp.example', 'service', 'main.aontu'), 'utf8'), /8080/);
        // A hand-vendored tree that names no version is taken as it is.
        write(Path.join(app, 'aontu_meta', 'vendor', 'corp.example', 'service'), {
            'pkg.aontu': 'pkg: {path: "corp.example/service", main: "main.aontu"}\n',
        });
        Fs.rmSync(Path.join(app, 'aontu_meta', 'vendor', 'corp.example', 'service', 'aontu_meta'), { recursive: true });
    ...
    (0, node_test_1.test)('get-raises-add-refuses-remove-drops', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
        ...
        Assert.match(Fs.readFileSync(Path.join(app, 'pkg.aontu'), 'utf8'), /v: "1.4.3"/);
        Assert.equal((0, pkg_1.readLock)(app)['corp.example/service'].v, '1.4.3');
        ...
        Assert.ok(!Fs.readFileSync(Path.join(app, 'pkg.aontu'), 'utf8').includes('service'));
        Assert.ok(!Fs.existsSync(Path.join(app, 'aontu_meta', 'vendor', 'corp.example')));
        Assert.deepEqual((0, pkg_1.readLock)(app), {});
        ...
        Assert.match(Fs.readFileSync(Path.join(app, 'pkg.aontu'), 'utf8'), /dep: "corp.example\/service": \{ v: "1.4.3" \}/);
    ...
    (0, node_test_1.test)('cooldown-holds-the-newest-version-back', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
        ...
        Assert.ok(!Fs.readFileSync(Path.join(app, 'pkg.aontu'), 'utf8').includes('corp.example/service'));
    ...
    (0, node_test_1.test)('outdated-lists-what-moves-and-what-was-retracted', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        ...
        Fs.rmSync(Path.join(w.repo, 'advisory', 'corp.example', 'common.aontu'));
        ...
        const three = await publish(w, await publisherWith(w, 'service', '1.2.0', '@"corp.example/common"\n@"corp.example/extra"\nname: string\n', '"corp.example/common": {v: "1.1.0"}, "corp.example/extra": {v: "1.0.0"}', 'retract: ["1.0.0", "1.1.0"]\n'));
        ...
        const advisory = readJson(Path.join(w.repo, 'advisory', 'corp.example', 'service.aontu'));
        Assert.deepEqual(advisory.retracted, [{ version: '1.0.0', by: '1.1.0' }, { version: '1.0.0', by: '1.2.0' },
            { version: '1.1.0', by: '1.2.0' }]);
    });
    (0, node_test_1.test)('a-closure-is-acquired-deps-first-and-an-alias-names-its-package', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'common', '1.0.0', 'x: 1\n'));
        ...
        Assert.equal(two.code, 0, two.out);
        const app = consumer(w, '"corp.example/service": {v: "2.0.0"}, "alias:legacy": {pkg: "corp.example/service", v: "1.0.0"}');
        Fs.writeFileSync(Path.join(app, 'main.aontu'), 'a: @"corp.example/service"\nb: @"alias:legacy"\na: name: "x"\nb: name: "y"\n');
        const r = await run(w, http, 'sync', [app]);
        Assert.equal(r.code, 0, r.out);
        const lock = (0, pkg_1.readLock)(app);
        ...
        Assert.ok(Fs.existsSync(Path.join(app, 'aontu_meta', 'vendor', 'alias', 'legacy', 'main.aontu')));
        ...
        const top = await publish(w, await publisherWith(w, 'top', '1.0.0', '@"corp.example/common"\n@"corp.example/mid"\n', '"corp.example/common": {v: "1.0.0"}, "corp.example/mid": {v: "1.0.0"}'));
        ...
        const overLock = (0, pkg_1.readLock)(Path.join(w.cache, 'aontu', 'pkg', 'store', (0, pkg_1.readLock)(Path.join(w.dir, 'app' + (consumers - 1)))['corp.example/over'].canon, 'corp.example', 'over'));
        Assert.deepEqual(Object.keys(overLock).sort(), ['corp.example/common', 'corp.example/mid']);
    });
    (0, node_test_1.test)('routing-and-trust-refusals', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
        ...
        Fs.writeFileSync(Path.join(other, 'pkg.aontu'), Fs.readFileSync(Path.join(other, 'pkg.aontu'), 'utf8').replace(KEY_ID, (0, pkg_net_1.keyIdFromPem)(OTHER_PEM)));
        ...
        Fs.writeFileSync(Path.join(inclusion, 'pkg.aontu'), Fs.readFileSync(Path.join(inclusion, 'pkg.aontu'), 'utf8').replace('inclusion: none', 'inclusion: required'));
        ...
        const config = (0, pkg_net_1.repoConfig)(consumer(w, dep, 'repo: {trust: {"corp.example/service": {signer: "' + KEY_ID + '", inclusion: none}, ' +
            '"corp.example/service/*": {signer: forge}, "corp.example/*": {signer: "' + KEY_ID + '"}}}\n'), (0, cli_1.pkgToolOptions)({ kind: 'system', textExt: [] }, w.dir));
        Assert.equal((0, pkg_net_1.trustEntryFor)(config, 'corp.example/service').signer, KEY_ID);
        Assert.equal((0, pkg_net_1.trustEntryFor)(config, 'corp.example/service').inclusion, 'none');
        Assert.equal((0, pkg_net_1.trustEntryFor)(config, 'corp.example/service/sub').signer, 'forge');
        Assert.equal((0, pkg_net_1.trustEntryFor)(config, 'corp.example/other').inclusion, 'required');
        Assert.equal((0, pkg_net_1.trustEntryFor)(config, 'other.example/x').signer, 'forge');
        Assert.deepEqual(config.base, ['https://pkg.aontu.dev']);
        Assert.equal((0, pkg_net_1.patternMatches)('corp.example/*', 'corp.example'), true);
        Assert.equal((0, pkg_net_1.patternMatches)('corp.example/x', 'corp.example/xy'), false);
        Assert.equal((0, pkg_net_1.baseAdmitted)('not a url'), false);
        Assert.equal((0, pkg_net_1.baseAdmitted)('http://localhost:8017'), true);
        Assert.equal((0, pkg_net_1.baseAdmitted)('http://[::1]:8017'), true);
        Assert.equal((0, pkg_net_1.isLoopback)('://'), false);
        Assert.equal((0, pkg_net_1.objectPath)('sigstore', 'corp.example/Svc', '1.0.0'), '/pkg/corp.example/!svc/@v/1.0.0.sigstore.json');
    });
    (0, node_test_1.test)('selection-refusals', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
        await publish(w, publisher(w, 'service', '1.4.3', SERVICE));
        ...
        Assert.equal((await run(w, http, 'sync', [app])).code, 0);
        write(Path.join(w.cache, 'aontu', 'pkg', 'seen', 'corp.example', 'service'), { '1.0.0.aontu': '{"package":"corp.example/service","version":"1.0.0"}\n' });
    ...
    (0, node_test_1.test)('a-move-refuses-and-never-redirects', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
    ...
    (0, node_test_1.test)('bytes-before-meaning-refusals', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
        const app = consumer(w, '"corp.example/service": {v: "1.4.2"}');
        ...
        Fs.rmSync(sigFile);
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: proof_missing: no proof is served for corp.example\/service 1.4.2/);
        Fs.writeFileSync(sigFile, 'not a proof\n');
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: proof_invalid: the proof for corp.example\/service 1.4.2: the proof is not an aontu-signature\/v1 key proof/);
        ...
        fresh();
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: archive_digest_mismatch: the archive for corp.example\/service 1.4.2 is sha256:/);
        Fs.rmSync(zipFile);
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: fetch_failed: no archive for corp.example\/service 1.4.2/);
        ...
        const pkgFile = Buffer.from('pkg: {path: "corp.example/service", version: "1.4.2", main: "main.aontu"}\n');
        const good = [
            { path: 'main.aontu', data: new Uint8Array(Buffer.from(SERVICE)) },
            { path: 'pkg.aontu', data: new Uint8Array(pkgFile) },
        ];
        serve([...good, { path: 'run.sh', data: new Uint8Array(Buffer.from('#!/bin/sh\n')) }]);
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: archive_entry_forbidden: .* carries run.sh, which the allowlist does not admit/);
        serve([...good, { path: '../x.aontu', data: new Uint8Array(1) }], (m) => {
            for (const f of m.archive.files) {
                f.path = '../x.aontu' === f.path ? 'x.aontu' : f.path;
            }
        });
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: archive_path_invalid: .* an entry path element is empty or begins or ends with a dot \(\.\.\/x.aontu\)/);
        serve([...good, { path: 'big.aontu', data: new Uint8Array(8388609) }]);
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: archive_bomb: .* unpacks past the size cap/);
        serve(good, (m) => { m.archive.files[0].digest = 'sha256:' + 'a'.repeat(64); });
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: file_manifest_mismatch: .* holds main.aontu, which the manifest does not list as served/);
        serve(good, (m) => { m.archive.files.push({ path: 'zzz.aontu', digest: 'sha256:' + 'a'.repeat(64), size: 1 }); });
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: file_manifest_mismatch: .* lacks zzz.aontu, which the manifest lists/);
        serve(good, (m) => { m.modules[0].canon = 'aon1-' + 'A'.repeat(43); });
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: module_integrity: corp.example\/service 1.4.2 means aon1-.*, and the manifest pins aon1-A+/);
        serve([good[0], { path: 'pkg.aontu', data: new Uint8Array(Buffer.from('pkg: {path: "corp.example/service", version: "1.4.1", main: "main.aontu"}\n')) }]);
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: manifest_invalid: the package file inside corp.example\/service 1.4.2 disagrees with the manifest/);
        serve([{ path: 'main.aontu', data: new Uint8Array(Buffer.from('a: 1\na: 2\n')) }, good[1]]);
        ...
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: manifest_invalid: .* declares alias:x without the package it names/);
        serve(good, (m) => { m.version = '1.4.9'; });
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: response_mismatch: the manifest served names corp.example\/service 1.4.9, not corp.example\/service 1.4.2/);
        serve(good, (m) => { m.schema = 'other'; });
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: manifest_invalid: the manifest for corp.example\/service 1.4.2: schema is not aontu-package\/v1/);
        // A zip that is not the canonical form, described honestly.
        const loose = Buffer.from((0, pkg_zip_1.zipCanonical)(good));
        ...
        resign(w, 'service', '1.4.2', (m) => {
            ...
            m.version = '1.4.2';
            m.deps = {};
            m.archive.digest = (0, pkg_zip_1.sha256Hex)(new Uint8Array(loose));
        });
        fresh();
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: archive_not_canonical: the archive for corp.example\/service 1.4.2: archive is not canonical/);
        const many = [];
        for (let i = 0; i < 4097; i++) {
            many.push({ path: 0 === i ? 'main.aontu' : 'f' + i + '.aontu', data: new Uint8Array(1) });
        }
        serve(many);
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: archive_too_many_files: /);
        const huge = new Uint8Array(16777217);
        Fs.writeFileSync(zipFile, huge);
        resign(w, 'service', '1.4.2', (m) => { m.archive.digest = (0, pkg_zip_1.sha256Hex)(huge); });
        fresh();
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: archive_too_large: /);
    });
    (0, node_test_1.test)('manifest-and-proof-shapes', async () => {
        const base = () => ({
            schema: 'aontu-package/v1', package: 'corp.example/x', version: '1.0.0',
            publish: 'private',
            archive: { format: 'zip', digest: 'sha256:' + 'a'.repeat(64), size: 1,
                files: [{ path: 'main.aontu', digest: 'sha256:' + 'b'.repeat(64), size: 1 }] },
            modules: [{ path: 'corp.example/x', main: 'main.aontu', canon: 'aon1-' + 'A'.repeat(43) }],
            deps: {}, published: '2026-01-01T00:00:00Z',
        });
        Assert.equal((0, pkg_net_1.manifestError)(base()), undefined);
        const cases = [
            ...
            [(m) => { m.modules[0].main = '../main.aontu'; }, /modules names an entry the archive does not hold/],
            [(m) => { m.modules[0].main = 'other.aontu'; }, /modules names an entry the archive does not hold/],
        ];
        for (const [edit, want] of cases) {
            const m = base();
            edit(m);
            Assert.match((0, pkg_net_1.manifestError)(m), want);
        }
        Assert.equal((0, pkg_net_1.manifestError)(null), 'schema is not aontu-package/v1');
        Assert.equal((0, pkg_net_1.manifestError)({ ...base(), deps: { 'corp.example/y': { v: '1.0.0', pkg: 'corp.example/z' } } }), undefined);
        Assert.equal((0, pkg_net_1.relPathError)(''), 'an entry path is empty, absolute or a directory');
        Assert.equal((0, pkg_net_1.relPathError)('a/'), 'an entry path is empty, absolute or a directory');
        Assert.equal((0, pkg_net_1.relPathError)('a b.aontu'), 'an entry path element is outside the alphabet');
        Assert.equal((0, pkg_net_1.relPathError)('a/.hidden/b.aontu'), 'an entry path element is empty or begins or ends with a dot');
        Assert.equal((0, pkg_net_1.relPathError)('a/b.aontu'), undefined);
        const digest = 'sha256:' + 'c'.repeat(64);
        const proof = (0, pkg_net_1.signDigest)(KEY_PEM, digest);
        Assert.equal((0, pkg_net_1.verifyKeyProof)(proof, digest, KEY_ID), undefined);
        Assert.match((0, pkg_net_1.verifyKeyProof)({ ...proof, kind: 'sigstore' }, digest, KEY_ID), /not an aontu-signature/);
        Assert.match((0, pkg_net_1.verifyKeyProof)(proof, digest, (0, pkg_net_1.keyIdFromPem)(OTHER_PEM)), /^signed by ed25519:/);
        Assert.match((0, pkg_net_1.verifyKeyProof)({ ...proof, signer: 'ed25519:' + 'A'.repeat(43) }, digest, 'ed25519:' + 'A'.repeat(43)), /the signer is a key of small order/);
        const keyOf = (hex) => 'ed25519:' + Buffer.from(hex, 'hex').toString('base64url');
        for (const hex of ['ec' + 'ff'.repeat(30) + '7f', 'ed' + 'ff'.repeat(30) + '7f',
            'ee' + 'ff'.repeat(30) + 'ff', '01' + '00'.repeat(30) + '80']) {
            Assert.equal((0, pkg_net_1.smallOrderKey)(Buffer.from(hex, 'hex')), true, hex);
            Assert.match((0, pkg_net_1.verifyKeyProof)({ ...proof, signer: keyOf(hex) }, digest, keyOf(hex)), /the signer is a key of small order/);
        }
        Assert.equal((0, pkg_net_1.smallOrderKey)(Buffer.from('02' + 'ff'.repeat(30) + '7f', 'hex')), false);
        Assert.equal((0, pkg_net_1.smallOrderKey)(Buffer.from('ed' + 'ff'.repeat(29) + 'fe7f', 'hex')), false);
        const B64 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';
        const slack = (text) => text.slice(0, -1) + B64[B64.indexOf(text[text.length - 1]) | 1];
        Assert.match((0, pkg_net_1.verifyKeyProof)({ ...proof, signature: slack(proof.signature) }, digest, KEY_ID), /malformed key or signature/);
        Assert.match((0, pkg_net_1.verifyKeyProof)({ ...proof, signer: slack(KEY_ID) }, digest, slack(KEY_ID)), /malformed key or signature/);
        Assert.match((0, pkg_net_1.verifyKeyProof)({ ...proof, signature: proof.signature.slice(0, 85) + '=' }, digest, KEY_ID), /malformed key or signature/);
        Assert.equal((0, pkg_net_1.timestamp)(new Date('2026-01-02T03:04:05.678Z')), '2026-01-02T03:04:05Z');
        Assert.match((0, pkg_net_1.verifyKeyProof)({ kind: 'key', encoding: 'aontu-signature/v1', over: digest, signature: 'x' }, digest, KEY_ID), /not an aontu-signature/);
        Assert.equal((0, pkg_net_1.objectPath)('latest', 'corp.example/x'), '/pkg/corp.example/x/@latest');
        const token = (claims) => 'h.' + Buffer.from(JSON.stringify(claims)).toString('base64url') + '.s';
        Assert.equal((0, pkg_net_1.publisherFromToken)(token({})), undefined);
        Assert.equal((0, pkg_net_1.publisherFromToken)(token({
        ...
        Assert.equal((0, pkg_net_1.servedUrl)('::1', 5), 'http://[::1]:5');
        Assert.equal((0, pkg_net_1.servedUrl)('127.0.0.1', 5), 'http://127.0.0.1:5');
        const waited = (0, cli_1.serveUntilInterrupted)();
        process.emit('SIGINT');
        await waited;
    });
    (0, node_test_1.test)('bounds-configurations-and-odd-objects', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        {
        ...
        pkg_net_1.LIMITS.closure = 1024;
        pkg_net_1.LIMITS.depth = 1;
        clean();
        ...
        write(Path.join(w.repo, 'advisory', 'corp.example'), { 'common.aontu': '{}\n' });
        const noAdvisory = await run(w, http, 'get', ['corp.example/common@1.0.0', consumer(w, '')]);
        Assert.equal(noAdvisory.code, 0, noAdvisory.out);
        write(Path.join(w.repo, 'tombstone', 'corp.example', 'common', '@v'), { '9.9.9.aontu': '{}\n' });
        ...
        {
            const r = await publish(w, aliased);
            Assert.equal(r.code, 0, r.out + r.err);
        }
        clean();
        ...
        resign(w, 'common', '1.0.0', (m) => { m.modules[0].main = 'nope.aontu'; });
        clean();
        ...
        Fs.writeFileSync(Path.join(frozenApp, 'pkg.aontu'), Fs.readFileSync(Path.join(frozenApp, 'pkg.aontu'), 'utf8').replace('1.0.0', '1.1.0'));
        Fs.rmSync(Path.join(frozenApp, 'aontu_meta', 'vendor'), { recursive: true, force: true });
        ...
        const noCache = { eval: (0, cli_1.pkgToolOptions)({ kind: 'system', textExt: [] }, w.dir).eval };
        const direct = await (0, pkg_net_1.pkgPublish)(publisher(w, 'direct', '1.0.0', 'x: 1\n', 'publish: public\n'), noCache, http, { yes: true, key: w.key, to: w.repo });
        ...
        const rootless = write(tmp('rootless'), { 'pkg.aontu': 'dep: {"corp.example/common": {v: "1.0.0"}}\n' });
        ...
        const svc2 = await publisherWith(w, 'service', '1.1.0', '@"corp.example/common"\n@"corp.example/extra"\nname: string\n', '"corp.example/common": {v: "1.0.0"}, "corp.example/extra": {v: "1.0.0"}');
        {
            const r = await publish(w, svc2);
            Assert.equal(r.code, 0, r.out + r.err);
        }
        backdate(w, 'service');
        backdate(w, 'extra');
    ...
    (0, node_test_1.test)('editing-the-package-file', () => {
        const w = world();
        const options = (0, cli_1.pkgToolOptions)({ kind: 'system', textExt: [] }, w.dir);
        const app = consumer(w, '"corp.example/service": {v: "1.4.2"}');
        Assert.match((0, pkg_net_1.editDeps)(app, { op: 'raise', key: 'corp.example/none', v: '1.0.0' }, options), /is not on one line of pkg.aontu; edit it by hand/);
        Fs.writeFileSync(Path.join(app, 'pkg.aontu'), 'dep: {\n  "corp.example/service": {\n    v: "1.4.2"\n  }\n}\n');
        Assert.match((0, pkg_net_1.editDeps)(app, { op: 'raise', key: 'corp.example/service', v: '1.4.3' }, options), /spans several lines of pkg.aontu; edit it by hand/);
        Assert.match((0, pkg_net_1.editDeps)(app, { op: 'remove', key: 'corp.example/service' }, options), /spans several lines of pkg.aontu; edit it by hand/);
        Assert.match(Fs.readFileSync(Path.join(app, 'pkg.aontu'), 'utf8'), /v: "1.4.2"/);
        Fs.writeFileSync(Path.join(app, 'pkg.aontu'), 'dep: {\n  "corp.example/service": {}\n}\n');
        Assert.match((0, pkg_net_1.editDeps)(app, { op: 'raise', key: 'corp.example/service', v: '1.4.3' }, options), /declares its version on another line of pkg.aontu; edit it by hand/);
        // An entry named in a comment is not an entry: the edit is undone.
        Fs.writeFileSync(Path.join(app, 'pkg.aontu'), '# "corp.example/service" was here\ndep: {"corp.example/service": {v: "1.4.2"}}\n');
        Assert.match((0, pkg_net_1.editDeps)(app, { op: 'remove', key: 'corp.example/service' }, options), /the edit to pkg.aontu did not take; edit it by hand/);
        Assert.match(Fs.readFileSync(Path.join(app, 'pkg.aontu'), 'utf8'), /^# "corp.example\/service" was here/);
        Fs.writeFileSync(Path.join(app, 'pkg.aontu'), 'dep: {');
        Assert.match((0, pkg_net_1.editDeps)(app, { op: 'add', key: 'corp.example/service', v: '1.4.2' }, options), /did not take/);
        Fs.writeFileSync(Path.join(app, 'pkg.aontu'), 'dep: {}');
        Assert.equal((0, pkg_net_1.editDeps)(app, { op: 'add', key: 'corp.example/service', v: '1.4.2' }, options), undefined);
        Assert.equal(Fs.readFileSync(Path.join(app, 'pkg.aontu'), 'utf8'), 'dep: {}\ndep: "corp.example/service": { v: "1.4.2" }\n');
        Assert.equal((0, pkg_net_1.editDeps)(app, { op: 'raise', key: 'corp.example/service', v: '1.4.3' }, options), undefined);
        Assert.equal((0, pkg_net_1.editDeps)(app, { op: 'remove', key: 'corp.example/service' }, options), undefined);
        Assert.equal(Fs.readFileSync(Path.join(app, 'pkg.aontu'), 'utf8'), 'dep: {}\n');
        Fs.rmSync(Path.join(app, 'pkg.aontu'));
        Assert.equal((0, pkg_net_1.editDeps)(app, { op: 'add', key: 'corp.example/service', v: '1.4.2' }, options), undefined);
        Assert.deepEqual((0, pkg_net_1.parsePkgSpec)('corp.example/x'), { pkg: 'corp.example/x' });
        Assert.deepEqual((0, pkg_net_1.parsePkgSpec)('corp.example/x@1.2.3'), { pkg: 'corp.example/x', version: '1.2.3' });
        Assert.match((0, pkg_net_1.parsePkgSpec)('x@1.2.3'), /not a package path/);
        Assert.match((0, pkg_net_1.parsePkgSpec)('corp.example/x@latest'), /not a version: latest/);
    });
    (0, node_test_1.test)('a-change-a-sync-cannot-carry-is-taken-back', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
        await publish(w, publisher(w, 'service', '1.4.3', SERVICE));
        const app = consumer(w, '"corp.example/service": {v: "1.4.2"}');
        Assert.equal((await run(w, http, 'sync', [app])).code, 0);
        const before = Fs.readFileSync(Path.join(app, 'pkg.aontu'), 'utf8');
        ...
        Assert.equal(Fs.readFileSync(Path.join(app, 'pkg.aontu'), 'utf8'), before);
        ...
        Assert.match(spans.err, /spans several lines of pkg.aontu; edit it by hand/);
        const spansRemove = await run(w, http, 'remove', ['corp.example/service', app]);
        Assert.equal(spansRemove.code, 2);
        Assert.match(spansRemove.err, /spans several lines of pkg.aontu/);
        Fs.writeFileSync(Path.join(app, 'pkg.aontu'), 'dep: {\n');
        ...
        const options = (0, cli_1.pkgToolOptions)({ kind: 'system', textExt: [] }, w.dir);
        Fs.rmSync(Path.join(w.cache, 'aontu'), { recursive: true, force: true });
        await Assert.rejects((0, pkg_net_1.pkgSync)(consumer(w, '"corp.example/service": {v: "1.4.2"}'), { ...options, cache: Path.join(w.cache, 'aontu', 'pkg') }, boom, {}), /boom/);
        ...
        Fs.appendFileSync(Path.join(kept, 'pkg.aontu'), 'dep: {"bad key!": {v: "1.0.0"}}\n');
        const lockBefore = lockOf(kept);
        const vendored = Path.join(kept, 'aontu_meta', 'vendor', 'corp.example', 'service', 'pkg.aontu');
        ...
        Assert.ok(!Fs.existsSync(Path.join(none, 'aontu_meta', 'pkg-lock.aontu')));
        ...
        write(Path.join(bad, 'aontu_meta', 'vendor', 'corp.example', 'service'), {
            'pkg.aontu': 'pkg: {path: "corp.example/service", version: "1.4.2", main: "main.aontu"}\n',
            'main.aontu': 'a: 1\na: 2\n',
        });
    ...
    (0, node_test_1.test)('publish-gates-and-refusals', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        ...
        const explicit = await publish(w, publisher(w, 'service', '1.6.0', SERVICE), ['--against', Path.join(w.dir, 'service-1.4.2')]);
        ...
        const sent = await run(w, answer(201, ''), 'publish', ['--yes', '--key', w.key, '--token', token, '--write', 'https://write.example', pub]);
        ...
        const coded = await run(w, answer(403, '{"code":"namespace_mismatch","message":"not yours"}'), 'publish', ['--yes', '--key', w.key, '--write', 'https://write.example', pub]);
        Assert.match(coded.out, /refused: namespace_mismatch: not yours/);
        const bare = await run(w, answer(500, 'oops'), 'publish', ['--yes', '--key', w.key, '--write', 'https://write.example', pub]);
        ...
        const received = [];
        const sink = (0, node_http_1.createServer)((req, res) => {
            let n = 0;
        ...
        const real = await run(w, (0, pkg_net_1.defaultHttp)(), 'publish', ['--yes', '--key', w.key, '--token', token, '--write', 'http://127.0.0.1:' + port, priv]);
        ...
        const dead = await run(w, (0, pkg_net_1.defaultHttp)(), 'publish', ['--yes', '--key', w.key, '--write', 'http://127.0.0.1:1', priv]);
        ...
        await Assert.rejects((0, pkg_net_1.pkgPublish)(priv, options, boom, { yes: true, key: w.key, write: 'https://x.example' }), /boom/);
        await Assert.rejects((0, pkg_net_1.pkgOutdated)(consumer(w, '"corp.example/service": {v: "1.4.2"}'), { ...options, cache: Path.join(w.cache, 'aontu', 'pkg') }, boom, {}).then(async () => {
            const app2 = consumer(w, '"corp.example/service": {v: "1.4.2"}');
            Assert.equal((await run(w, http, 'sync', [app2])).code, 0);
            await (0, pkg_net_1.pkgOutdated)(app2, { ...options, cache: Path.join(w.cache, 'aontu', 'pkg') }, boom, {});
        }), /boom/);
        // The publisher block from either forge, or none.
        Assert.equal((0, pkg_net_1.publisherFromToken)('nope'), undefined);
        Assert.equal((0, pkg_net_1.publisherFromToken)('a.!!.c'), undefined);
        Assert.equal((0, pkg_net_1.publisherFromToken)('a.' + Buffer.from('{"iss":"https://other"}').toString('base64url') + '.c'), undefined);
        const gl = (0, pkg_net_1.publisherFromToken)('a.' + Buffer.from(JSON.stringify({
        ...
        const bareGl = (0, pkg_net_1.publisherFromToken)('a.' + Buffer.from(JSON.stringify({
            iss: 'https://gitlab.com', project_path: 'corp/svc', namespace_id: 3, project_id: 4,
        })).toString('base64url') + '.c');
        Assert.equal(bareGl.workflow, undefined);
        const bareGh = (0, pkg_net_1.publisherFromToken)('a.' + Buffer.from(JSON.stringify({
        ...
        const untok = await run(w, answer(200, ''), 'publish', ['--yes', '--key', w.key, '--token', token, '--write', 'https://write.example', pub]);
        Assert.equal(untok.code, 0);
        Assert.equal(JSON.parse(Buffer.from(posted.parts.manifest).toString('utf8')).publisher, undefined);
    });
    (0, node_test_1.test)('publish-reads-the-predecessor-it-can-reach', async () => {
        const w = world();
    ...
    (0, node_test_1.test)('the-local-registry-serves-and-proxies', async () => {
        const w = world();
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
        const origin = await (0, pkg_net_1.startServe)({ dir: w.repo, upstream: [], listen: '127.0.0.1:0', http: (0, pkg_net_1.defaultHttp)() });
        const cacheDir = Path.join(w.dir, 'proxy');
        Fs.mkdirSync(cacheDir);
        const proxy = await (0, pkg_net_1.startServe)({ dir: cacheDir, upstream: [origin.url], listen: '127.0.0.1:0', http: (0, pkg_net_1.defaultHttp)() });
        (0, node_test_1.after)(async () => {
            await proxy.close();
            await origin.close();
        });
        const http = (0, pkg_net_1.defaultHttp)();
        const list = await http.get(origin.url + (0, pkg_net_1.objectPath)('list', 'corp.example/service'));
        ...
        const r = await fetch(origin.url + (0, pkg_net_1.objectPath)('archive', 'corp.example/service', '1.4.2'));
        Assert.equal(r.headers.get('content-type'), 'application/zip');
        Assert.equal(r.headers.get('cache-control'), 'max-age=31536000, immutable');
        const head = await fetch(origin.url + (0, pkg_net_1.objectPath)('list', 'corp.example/service'), { method: 'HEAD' });
        Assert.equal(head.status, 200);
        Assert.equal(head.headers.get('cache-control'), 'max-age=60');
        Assert.equal((await fetch(origin.url + '/', { method: 'POST' })).status, 405);
        const sig = await fetch(origin.url + (0, pkg_net_1.objectPath)('signature', 'corp.example/service', '1.4.2'));
        ...
        // served and says so.
        const app = consumer(w, '"corp.example/service": {v: "1.4.2"}', 'repo: {base: ["' + proxy.url + '"], trust: {"corp.example/*": {signer: "' + KEY_ID + '", inclusion: none}}}\n');
        const sync = await run(w, http, 'sync', [app]);
        Assert.equal(sync.code, 0, sync.out);
        ...
        const missing = await http.get(proxy.url + (0, pkg_net_1.objectPath)('list', 'corp.example/other'));
        Assert.equal(missing.status, 404);
        await origin.close();
        const stale = await fetch(proxy.url + (0, pkg_net_1.objectPath)('list', 'corp.example/service'));
        Assert.equal(stale.status, 200);
        Assert.match(stale.headers.get('x-aontu-stale'), /no upstream answered/);
        const served = await (0, pkg_net_1.serveObject)({ dir: cacheDir, upstream: ['http://127.0.0.1:1'], listen: '', http }, (0, pkg_net_1.objectPath)('list', 'corp.example/service')).catch((e) => e);
        Assert.ok(served instanceof Error || 200 === served.status);
        Assert.equal((0, pkg_net_1.objectShape)('/pkg/corp.example/x/@v/list'), true);
        Assert.equal((0, pkg_net_1.objectShape)('/pkg/corp.example/x/@v/1.0.0.sigstore.json'), true);
        Assert.equal((0, pkg_net_1.objectShape)('/tombstone/feed.aontu'), true);
        Assert.equal((0, pkg_net_1.objectShape)('/pkg/corp.example//x/@v/list'), false);
        Assert.equal((0, pkg_net_1.objectShape)('/pkg/corp.example/x/@v/1.0.zip'), false);
        Assert.deepEqual((0, pkg_net_1.splitListen)('127.0.0.1:8017'), ['127.0.0.1', 8017]);
        Assert.deepEqual((0, pkg_net_1.splitListen)('localhost'), ['localhost', 8017]);
        Assert.deepEqual((0, pkg_net_1.splitListen)('localhost:x'), ['localhost', 8017]);
        Assert.deepEqual((0, pkg_net_1.splitListen)('[::1]:8018'), ['::1', 8018]);
        Assert.deepEqual((0, pkg_net_1.splitListen)('[::1]'), ['::1', 8017]);
        Assert.deepEqual((0, pkg_net_1.splitListen)('[::1]:x'), ['::1', 8017]);
        const v6 = await (0, pkg_net_1.startServe)({ dir: w.repo, upstream: [], listen: '[::1]:0', http }).catch(() => undefined);
        if (undefined !== v6) {
        ...
        let seen;
        const cli = await run(w, http, 'pkg', ['serve', '--listen', '127.0.0.1:0', '--upstream', origin.url, w.repo], async (s) => { seen = s; });
        Assert.equal(cli.code, 0, cli.err);
        Assert.match(cli.out, /^serving .* at http:\/\/127.0.0.1:\d+\nupstream: http:\/\/127.0.0.1:\d+\n$/);
        Assert.ok(undefined !== seen);
    });
    (0, node_test_1.test)('the-verbs-take-their-arguments', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        const app = consumer(w, '');
        ...
        write(app, { 'aontu_meta/pkg-lock.aontu': '{"lock":{"corp.example/none":{"archive":"","canon":"","v":"1.0.0"},"bad key":{"archive":"","canon":"","v":"1.0.0"}}}\n' });
        ...
        try {
            (0, cli_1.main)(['node', 'cli', 'why', 'corp.example/x', app], servers(http));
    ...
    (0, node_test_1.test)('keygen-writes-a-key-once', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        ...
        Assert.equal((0, pkg_net_1.keyIdFromPem)(Fs.readFileSync(file, 'utf8')), made.out.slice('signer: '.length).trim());
    ...
    (0, node_test_1.test)('the-layout-writer-and-the-directory-reader', async () => {
        const w = world();
        const options = (0, cli_1.pkgToolOptions)({ kind: 'system', textExt: [] }, w.dir);
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        ...
        (0, pkg_net_1.writeLayout)(w.repo, { manifest, manifestBytes: bytes, proofBytes: new Uint8Array(1), archive: new Uint8Array(1) }, options, new Date('2026-01-01T00:00:00Z'));
        Assert.ok(Fs.existsSync(Path.join(w.repo, 'pkg', 'corp.example', '!svc', '@v', '1.0.0.zip')));
        Assert.throws(() => (0, pkg_net_1.writeLayout)(w.repo, { manifest, manifestBytes: bytes, proofBytes: new Uint8Array(1), archive: new Uint8Array(1) }, options, new Date()), (e) => e instanceof pkg_net_1.PkgRefusal && 'version_exists' === e.code);
        for (const odd of [{ ...manifest, package: '../../escape' }, { ...manifest, version: '../x' }]) {
            Assert.throws(() => (0, pkg_net_1.writeLayout)(w.repo, { manifest: odd, manifestBytes: bytes, proofBytes: new Uint8Array(1), archive: new Uint8Array(1) }, options, new Date()), (e) => e instanceof pkg_net_1.PkgRefusal && 'manifest_invalid' === e.code);
        }
        Assert.equal(pkg_net_1.COOLDOWN_HOURS, 72);
        // `why` reads the store beside the vendor tree.
        const app = consumer(w, '"corp.example/service": {v: "1.4.2"}');
        write(app, { 'aontu_meta/pkg-lock.aontu': '{"lock":{"corp.example/service":{"archive":"","canon":"","v":"1.4.2"},"bad key":{"archive":"","canon":"","v":"1.0.0"}}}\n' });
        const why = (0, pkg_net_1.pkgWhy)(app, options, 'corp.example/service');
        ...
        write(Path.join(app, 'aontu_meta', 'vendor', 'corp.example', 'service'), { 'pkg.aontu': 'pkg: {path: "corp.example/service"}\ndep: {"corp.example/common": {v: "1.0.0"}}\n' });
        write(Path.join(app, 'aontu_meta', 'vendor', 'corp.example', 'common'), { 'pkg.aontu': 'pkg: {path: "corp.example/common"}\ndep: {"corp.example/service": {v: "1.0.0"}}\n' });
        write(app, { 'aontu_meta/pkg-lock.aontu': '{"lock":{"corp.example/service":{"archive":"","canon":"","v":"1.4.2"},"corp.example/common":{"archive":"","canon":"","v":"1.0.0"}}}\n' });
        ...
        Assert.match(Fs.readFileSync(Path.join(w.repo, 'advisory', 'corp.example', 'ret.aontu'), 'utf8'), /"retracted":\[\{"by":"1.0.2","version":"1.0.0"\},\{"by":"1.0.2","version":"1.0.1"\}\]/);
        ...
        Assert.throws(() => (0, pkg_net_1.writeLayout)(w.repo, { manifest: { ...manifest, version: '1.2.0' }, manifestBytes: bytes, proofBytes: new Uint8Array(1), archive: new Uint8Array(1) }, options, new Date()), (e) => e instanceof pkg_net_1.PkgRefusal && 'path_moved' === e.code);
        const sync = await (0, pkg_net_1.pkgSync)(app, options, http, { base: ['ftp://x'] });
        Assert.equal(sync.verdict, 'refused');
    });
    (0, node_test_1.test)('every-listed-version-is-seen-and-a-tombstone-is-not-a-rollback', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
        await publish(w, publisher(w, 'service', '1.4.3', SERVICE));
        ...
        Assert.deepEqual(Fs.readdirSync(seen).sort(), ['1.4.2.aontu', '1.4.3.aontu']);
        ...
        Fs.writeFileSync(listFile, JSON.stringify({
            ...list, versions: list.versions.filter((e) => '1.4.3' !== e.version),
        }) + '\n');
        ...
        write(Path.join(w.repo, 'tombstone', 'corp.example', 'service', '@v'), { '1.4.3.aontu': '{"reason": "malware"}\n' });
        ...
        write(Path.join(w.repo, 'pkg', 'corp.example', 'empty', '@v'), { list: '{"package":"corp.example/empty","versions":[]}\n' });
        ...
        Assert.ok(!Fs.existsSync((0, mod_1.cacheSeenDir)(Path.join(w.cache, 'aontu', 'pkg'), 'corp.example/empty')));
        // A lock written without its header line is read the same under --frozen.
        const lockFile = Path.join(app, 'aontu_meta', 'pkg-lock.aontu');
        Fs.writeFileSync(lockFile, lockOf(app).split('\n').filter((l) => !l.startsWith('#')).join('\n'));
        const frozen = await run(w, http, 'sync', ['--frozen', app]);
        Assert.equal(frozen.code, 0, frozen.out);
    });
    (0, node_test_1.test)('a-frozen-refusal-prunes-nothing-and-a-vendored-tree-is-the-package-asked-for', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
        ...
        const pkgFile = Path.join(pair, 'pkg.aontu');
        ...
        write(alias, {
            ...
            'main.aontu': 'svc: @"alias:svc"\n',
        });
        Assert.equal((await run(w, http, 'sync', [alias])).code, 0);
        const aliasPkg = Path.join(alias, 'pkg.aontu');
        ...
        Assert.equal((0, pkg_1.readLock)(alias)['alias:svc'].pkg, 'corp.example/other');
        Assert.match(Fs.readFileSync(Path.join((0, mod_1.moduleDir)(Path.join(alias, 'aontu_meta', 'vendor'), 'alias:svc'), 'pkg.aontu'), 'utf8'), /path: "corp.example\/other"/);
        ...
        Fs.rmSync(Path.join(vend, 'aontu_meta', 'vendor', 'corp.example', 'service', 'aontu_meta', 'manifest.aontu'));
        ...
        Assert.deepEqual(JSON.parse(v.out).mismatched, [{
                key: 'corp.example/service', pin: 'manifest', want: (0, pkg_1.readLock)(vend)['corp.example/service'].manifest, got: '',
            }]);
    });
    (0, node_test_1.test)('the-signing-key-is-ed25519-and-the-transport-reads-to-the-cap', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        const tree = publisher(w, 'service', '1.4.2', SERVICE);
        const rsa = Path.join(w.dir, 'rsa.pem');
        Fs.writeFileSync(rsa, (0, node_crypto_1.generateKeyPairSync)('rsa', { modulusLength: 1024 }).privateKey
            .export({ format: 'pem', type: 'pkcs8' }));
        Assert.match((await run(w, http, 'publish', ['--key', rsa, '--to', w.repo, tree])).out, /refused: key_invalid: the key is rsa, not ed25519/);
        Fs.writeFileSync(rsa, 'not pem\n');
        Assert.match((await run(w, http, 'publish', ['--key', rsa, '--to', w.repo, tree])).out, /refused: key_invalid: the key file is not a PEM private key/);
        Assert.throws(() => (0, pkg_net_1.keyIdFromPem)('nope'), (e) => e instanceof pkg_net_1.PkgRefusal && 'key_invalid' === e.code);
        // A body is read no further than the archive cap.
        const big = (0, node_http_1.createServer)((_req, res) => {
            res.writeHead(200);
            res.write(Buffer.alloc(3000));
            setTimeout(() => res.end(Buffer.alloc(3000)), 50);
        });
        await new Promise((r) => big.listen(0, '127.0.0.1', () => r()));
        const url = 'http://127.0.0.1:' + big.address().port + '/x';
        const saved = pkg_1.ARCHIVE_LIMITS.bytes;
        pkg_1.ARCHIVE_LIMITS.bytes = 1000;
        const capped = await (0, pkg_net_1.defaultHttp)().get(url);
        pkg_1.ARCHIVE_LIMITS.bytes = saved;
        Assert.equal(capped.status, 200);
        Assert.equal(capped.body.length, 3000);
        const whole = await (0, pkg_net_1.defaultHttp)().get(url);
        Assert.equal(whole.body.length, 6000);
        await new Promise((r) => big.close(() => r()));
        Assert.equal((await (0, pkg_net_1.readBounded)(new Response(null), 10)).length, 0);
    });
    (0, node_test_1.test)('outdated-walks-the-whole-closure-that-moves', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        ...
        const moves = (key) => report.locked.find((e) => key === e.key).moves;
        Assert.deepEqual(moves('corp.example/service'), ['alias:c unlocked -> 1.2.0', 'corp.example/base unlocked -> 1.1.0', 'corp.example/common 1.0.0 -> 1.2.0']);
        ...
        Assert.deepEqual(odd.locked.find((e) => 'corp.example/service' === e.key).moves, [
        ...
        const bounds = { ...pkg_net_1.LIMITS };
        try {
            pkg_net_1.LIMITS.depth = 0;
            Assert.match((await run(w, http, 'pkg', ['outdated', app])).out, /refused: module_depth/);
            pkg_net_1.LIMITS.depth = bounds.depth;
            pkg_net_1.LIMITS.closure = 0;
            Assert.match((await run(w, http, 'pkg', ['outdated', app])).out, /refused: closure_too_large/);
        }
        finally {
            Object.assign(pkg_net_1.LIMITS, bounds);
        }
    });
    (0, node_test_1.test)('the-meaning-is-checked-against-the-pin-and-a-cycle-in-why-ends', async () => {
        const w = world();
        const http = (0, pkg_net_1.dirHttp)(w.repo);
        await publish(w, publisher(w, 'service', '1.4.2', SERVICE));
        const fresh = () => Fs.rmSync(Path.join(w.cache, 'aontu'), { recursive: true, force: true });
        ...
        fresh();
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: module_integrity: corp.example\/service 1.4.2 means aon1-[A-Za-z0-9_-]{43}, and the manifest pins aon1-A{43}/);
        // The module does not evaluate at all.
        const entries = [
            { path: 'main.aontu', data: new Uint8Array(Buffer.from('a: 1\na: 2\n')) },
            { path: 'pkg.aontu', data: new Uint8Array(Buffer.from('pkg: {path: "corp.example/service", version: "1.4.2", main: "main.aontu"}\n')) },
        ];
        const zip = (0, pkg_zip_1.zipCanonical)(entries);
        ...
        resign(w, 'service', '1.4.2', (m) => {
            m.archive.digest = (0, pkg_zip_1.sha256Hex)(zip);
            m.archive.size = zip.length;
            m.archive.files = entries.map((e) => ({ path: e.path, digest: (0, pkg_zip_1.sha256Hex)(e.data), size: e.data.length }));
        });
        fresh();
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: module_integrity: corp.example\/service 1.4.2 means nothing \(it does not evaluate\)/);
        ...
        write(cyc, {
            ...
            'main.aontu': 'x: 1\n',
            'aontu_meta/vendor/corp.example/a/pkg.aontu': 'pkg: {path: "corp.example/a", version: "1.0.0", main: "main.aontu"}\n' +
                'dep: {"corp.example/b": {v: "1.0.0"}, "corp.example/c": {v: "1.0.0"}}\n',
            'aontu_meta/vendor/corp.example/a/main.aontu': 'a: 1\n',
            'aontu_meta/vendor/corp.example/b/pkg.aontu': 'pkg: {path: "corp.example/b", version: "1.0.0", main: "main.aontu"}\ndep: {"corp.example/a": {v: "1.0.0"}}\n',
            'aontu_meta/vendor/corp.example/b/main.aontu': 'b: 1\n',
            'aontu_meta/pkg-lock.aontu': '{"lock":{"corp.example/a":' + entry('aon1-' + 'A'.repeat(43)) +
...
//# sourceMappingURL=pkgnet.test.js.map