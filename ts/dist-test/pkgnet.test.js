...
(0, node_test_1.describe)('pkg-net', () => {
    (0, node_test_1.test)('publish-then-sync-round-trip', async () => {
        ...
        Assert.ok(Fs.existsSync(Path.join(w.cache, 'aontu', 'pkg', 'seen', 'corp.example', 'service', '1.4.2.aon')));
        Assert.ok(Fs.existsSync(Path.join(w.cache, 'aontu', 'pkg', 'store', lock.canon, 'corp.example', 'service', 'main.aon')));
    ...
    (0, node_test_1.test)('frozen-refuses-a-lock-that-would-change', async () => {
        ...
        Fs.writeFileSync(Path.join(app, 'pkg.aon'), Fs.readFileSync(Path.join(app, 'pkg.aon'), 'utf8').split('\n')
    ...
    (0, node_test_1.test)('sync-replaces-a-vendor-tree-at-another-version', async () => {
        ...
        Assert.match(Fs.readFileSync(Path.join(app, 'aontu_meta', 'vendor', 'corp.example', 'service', 'main.aon'), 'utf8'), /8080/);
    ...
    (0, node_test_1.test)('get-raises-add-refuses-remove-drops', async () => {
        ...
        Assert.match(Fs.readFileSync(Path.join(app, 'pkg.aon'), 'utf8'), /dep: "corp.example\/service": \{ v: "1.4.3" \}/);
    ...
    (0, node_test_1.test)('a-closure-is-acquired-deps-first-and-an-alias-names-its-package', async () => {
        ...
        Fs.writeFileSync(Path.join(app, 'main.aon'), 'a: @"corp.example/service"\nb: @"alias:legacy"\na: name: "x"\nb: name: "y"\n');
        const r = await run(w, http, 'sync', [app]);
        Assert.equal(r.code, 0, r.out);
    ...
    (0, node_test_1.test)('routing-and-trust-refusals', async () => {
        ...
        Fs.writeFileSync(Path.join(other, 'pkg.aon'), Fs.readFileSync(Path.join(other, 'pkg.aon'), 'utf8').replace(KEY_ID, (0, pkg_net_1.keyIdFromPem)(OTHER_PEM)));
        ...
        Fs.writeFileSync(Path.join(inclusion, 'pkg.aon'), Fs.readFileSync(Path.join(inclusion, 'pkg.aon'), 'utf8').replace('inclusion: none', 'inclusion: required'));
    ...
    (0, node_test_1.test)('selection-refusals', async () => {
        ...
        write(Path.join(w.repo, 'tombstone', 'corp.example', 'service', '@v'), { '1.4.3.aon': '{"package":"corp.example/service","version":"1.4.3","reason":"malware"}\n' });
    ...
    (0, node_test_1.test)('bytes-before-meaning-refusals', async () => {
        ...
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: archive_path_invalid: .* an entry path element is empty or begins or ends with a dot \(\.\.\/x.aon\)/);
        ...
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: file_manifest_mismatch: .* holds main.aon, which the manifest does not list as served/);
        serve(good, (m) => { m.archive.files.push({ path: 'zzz.aon', digest: 'sha256:' + 'a'.repeat(64), size: 1 }); });
        Assert.match((await run(w, http, 'sync', [app])).out, /refused: file_manifest_mismatch: .* lacks zzz.aon, which the manifest lists/);
        ...
        serve([good[0], { path: 'pkg.aon', data: new Uint8Array(Buffer.from('pkg: {path: "corp.example/service", version: "1.4.1", main: "main.aon"}\n')) }]);
    ...
    (0, node_test_1.test)('manifest-and-proof-shapes', async () => {
        ...
        Assert.equal((0, pkg_net_1.relPathError)('a b.aon'), 'an entry path element is outside the alphabet');
        Assert.equal((0, pkg_net_1.relPathError)('a/.hidden/b.aon'), 'an entry path element is empty or begins or ends with a dot');
        Assert.equal((0, pkg_net_1.relPathError)('a/b.aon'), undefined);
    ...
    (0, node_test_1.test)('bounds-configurations-and-odd-objects', async () => {
        ...
        Fs.writeFileSync(Path.join(frozenApp, 'pkg.aon'), Fs.readFileSync(Path.join(frozenApp, 'pkg.aon'), 'utf8').replace('1.0.0', '1.1.0'));
        Fs.rmSync(Path.join(frozenApp, 'aontu_meta', 'vendor'), { recursive: true, force: true });
    ...
    (0, node_test_1.test)('editing-the-package-file', () => {
        ...
        const app = consumer(w, '"corp.example/service": {v: "1.4.2"}');
        Assert.match((0, pkg_net_1.editDeps)(app, { op: 'raise', key: 'corp.example/none', v: '1.0.0' }, options), /is not on one line of pkg.aon; edit it by hand/);
        Fs.writeFileSync(Path.join(app, 'pkg.aon'), 'dep: {\n  "corp.example/service": {\n    v: "1.4.2"\n  }\n}\n');
        Assert.match((0, pkg_net_1.editDeps)(app, { op: 'raise', key: 'corp.example/service', v: '1.4.3' }, options), /spans several lines of pkg.aon; edit it by hand/);
        Assert.match((0, pkg_net_1.editDeps)(app, { op: 'remove', key: 'corp.example/service' }, options), /spans several lines of pkg.aon; edit it by hand/);
        Assert.match(Fs.readFileSync(Path.join(app, 'pkg.aon'), 'utf8'), /v: "1.4.2"/);
        Fs.writeFileSync(Path.join(app, 'pkg.aon'), 'dep: {\n  "corp.example/service": {}\n}\n');
        Assert.match((0, pkg_net_1.editDeps)(app, { op: 'raise', key: 'corp.example/service', v: '1.4.3' }, options), /declares its version on another line of pkg.aon; edit it by hand/);
        // An entry named in a comment is not an entry: the edit is undone.
        Fs.writeFileSync(Path.join(app, 'pkg.aon'), '# "corp.example/service" was here\ndep: {"corp.example/service": {v: "1.4.2"}}\n');
        Assert.match((0, pkg_net_1.editDeps)(app, { op: 'remove', key: 'corp.example/service' }, options), /the edit to pkg.aon did not take; edit it by hand/);
    ...
    (0, node_test_1.test)('the-local-registry-serves-and-proxies', async () => {
        ...
        Assert.equal((0, pkg_net_1.objectShape)('/tombstone/feed.aon'), true);
    ...
    (0, node_test_1.test)('the-layout-writer-and-the-directory-reader', async () => {
        ...
        write(Path.join(app, 'aontu_meta', 'vendor', 'corp.example', 'service'), { 'pkg.aon': 'pkg: {path: "corp.example/service"}\ndep: {"corp.example/common": {v: "1.0.0"}}\n' });
        write(Path.join(app, 'aontu_meta', 'vendor', 'corp.example', 'common'), { 'pkg.aon': 'pkg: {path: "corp.example/common"}\ndep: {"corp.example/service": {v: "1.0.0"}}\n' });
    ...
    (0, node_test_1.test)('a-frozen-refusal-prunes-nothing-and-a-vendored-tree-is-the-package-asked-for', async () => {
        ...
        Assert.match(Fs.readFileSync(Path.join((0, mod_1.moduleDir)(Path.join(alias, 'aontu_meta', 'vendor'), 'alias:svc'), 'pkg.aon'), 'utf8'), /path: "corp.example\/other"/);
