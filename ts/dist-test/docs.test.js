...
(0, node_test_1.test)('the-class-table-and-the-totals-count-the-registry', () => {
    ...
    const counted = new Map(Array.from(section(text, '## Classes').matchAll(/^\| `([a-z]+)` \| (\d+) \|/gm), (row) => [row[1], Number(row[2])]));
    Assert.deepStrictEqual(Object.fromEntries([...counted].sort()), Object.fromEntries([...byClass].sort()), 'the class table must count the registry');
    ...
    Assert.equal(stated(/registry holds \*\*(\d+)\*\* codes/, 'registry total'), total, 'the stated registry total must count the registry');
    Assert.equal(stated(/All (\d+) codes have hint text/, 'hint-text total'), total, 'the stated hint-text total must count the registry');
    ...
    Assert.ok(byClass.size < WORD.length, 'no word for a registry of ' + byClass.size + ' classes');
    Assert.equal(named[1], WORD[byClass.size], 'the stated class count must count the registry');
});
...
(0, node_test_1.test)('the-package-refusals-are-the-sources', () => {
    const raised = refusalCodes();
    Assert.ok(20 < raised.length, 'no refusal codes read from the source');
    const rows = Array.from(section(docsText('reference-packages.md'), '## Refusals')
        .matchAll(/^\| `([a-z_]+)` \|/gm), (m) => m[1]);
    Assert.deepStrictEqual(rows, raised, 'the refusal table must list every code a package operation raises');
});
(0, node_test_1.test)('the-package-caps-are-the-engines', () => {
    const src = (f) => Fs.readFileSync(Path.join(REPO, 'ts', 'src', f), 'utf8');
    const num = (text, re) => {
    ...
    const rows = Array.from(section(docsText('reference-packages.md'), '## The caps')
        .matchAll(/^\| [^|]+ \| (\d+)[^|]*\|/gm), (m) => m[1]);
    Assert.deepStrictEqual(rows, caps, 'the caps table must carry the engine numbers, in order');
});
...
function abnfRules() {
    const parts = Fs.readFileSync(Path.join(REPO, 'grammar', 'aontu.abnf'), 'utf8')
...
function abnfRule(name) {
    const body = abnfRules().get(name);
    Assert.ok(null != body, 'no rule ' + name + ' in aontu.abnf');
    return body;
}
(0, node_test_1.test)('the-teaching-topics-are-the-binarys', () => {
    const carried = JSON.parse((0, node_child_process_1.execFileSync)(process.execPath, [CLI, 'help', '--format', 'json'], { encoding: 'utf8' })).topics.map((t) => t.topic);
    Assert.ok(3 < carried.length, 'no topics read from the binary');
    const rows = Array.from(section(docsText('reference-agents.md'), '## The teaching topics')
        .matchAll(/^\| `([a-z]+)` \|/gm), (m) => m[1]);
    Assert.deepStrictEqual(rows, carried, 'the topic table must be what the binary carries, in order');
});
// Every verb that answers JSON, with a run that reaches its report.
// The shape is read off the answer, never assumed: that is the whole
// point of holding the page to it.
const JSON_RUNS = [
    ['the bare entry point', (d) => [d.doc]],
    ['model get', (d) => ['model', 'get', '$', d.doc]],
    ['model why', (d) => ['model', 'why', '$.a', d.doc]],
    ['vet', (d) => ['vet', d.schema, d.doc]],
    ['subsume', (d) => ['subsume', d.schema, d.doc]],
    ['breaking', (d) => ['breaking', '--against', d.doc, d.doc]],
    ['relations', (d) => ['relations', d.rel]],
    ['reaches', (d) => ['reaches', '$.a', '$.a', d.rel]],
    ['trim', (d) => ['trim', '--check', d.doc]],
    ['jsonschema', (d) => ['jsonschema', d.schema]],
    ['view', (d) => ['view', 'tree', d.doc]],
    ['render --check', (d) => ['render', '--check', d.gen, d.out]],
    ['allow', (d) => ['allow', '--role', 'dev', d.roles, '$.a']],
    ['hash', (d) => ['hash', d.doc]],
    ['help', () => ['help']],
    ['explain', () => ['explain', '--list']],
];
function jsonVerbFixtures() {
    const dir = Fs.mkdtempSync(Path.join(Os.tmpdir(), 'aontu-json-'));
    const at = (name, body) => {
...
function jsonAnswer(argv) {
    const run = () => {
        try {
            return (0, node_child_process_1.execFileSync)(process.execPath, [CLI, ...argv, '--format', 'json'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
        }
        catch (e) {
            return e.stdout ?? '';
        }
...
// Which row of a table in the page a verb must be named in.
function rowNaming(table, marker, verb) {
    const row = table.split('\n')
        .filter((l) => l.startsWith('| '))
        .find((l) => l.includes(marker));
    Assert.ok(null != row, 'no row for ' + marker);
    const named = 'the bare entry point' === verb
        ? row.includes(verb)
        : row.includes('`' + verb + '`');
    Assert.ok(named, verb + ' belongs in the row for ' + marker + ': ' + row);
}
(0, node_test_1.test)('the-machine-answer-shapes-are-the-clis', () => {
    const shapes = section(docsText('reference-agents.md'), '## The machine answer');
    const fixtures = jsonVerbFixtures();
    const SHAPE = {
        ok: '`ok` and the answer beside it',
        verdict: 'a verdict word',
        payload: 'the payload alone',
    };
    const FINDINGS = {
        findings: '| `findings` |',
        errors: '| `errors` |',
        none: 'the verb reports none',
    };
    for (const [verb, argv] of JSON_RUNS) {
        const answer = jsonAnswer(argv(fixtures));
        const keys = Object.keys(answer);
        Assert.ok(keys.includes('aontu'), verb + ' answers no aontu block');
        rowNaming(shapes, SHAPE[keys.includes('ok') ? 'ok'
            : keys.includes('verdict') ? 'verdict' : 'payload'], verb);
        rowNaming(shapes, FINDINGS[keys.includes('findings') ? 'findings'
            : keys.includes('errors') ? 'errors' : 'none'], verb);
    }
    // The stated exception, and the stated refusals.
    Assert.deepStrictEqual(Object.keys(jsonAnswer(['trace', fixtures.gen])), ['trace'], 'trace answers its record alone');
...
(0, node_test_1.test)('the-grammar-rule-index-is-the-published-file', () => {
    const defined = [...abnfRules().keys()];
    Assert.ok(20 < defined.length, 'no rules read from the grammar');
    const rows = Array.from(section(docsText('reference-grammar.md'), '## The rules')
        .matchAll(/^\| `([A-Za-z][A-Za-z0-9-]*)` \|/gm), (m) => m[1]);
    Assert.deepStrictEqual(rows, defined, 'the rule index must be the grammar file, in the order it defines them');
});
(0, node_test_1.test)('the-shared-grammar-rules-are-shared', () => {
    const names = (file, re) => new Set(Array.from(Fs.readFileSync(Path.join(REPO, 'grammar', file), 'utf8')
        .matchAll(re), (m) => m[1]));
    const abnf = new Set(abnfRules().keys());
    const gbnf = names('aontu.gbnf', /^([A-Za-z][A-Za-z0-9-]*)[ \t]*::=/gm);
    const lark = names('aontu.lark', /^([A-Za-z_][A-Za-z0-9_]*)[ \t]*:/gm);
    Assert.ok(20 < gbnf.size, 'no rules read from the gbnf grammar');
    Assert.deepStrictEqual([...lark].sort(), [...gbnf].sort(), 'the gbnf and lark grammars must name the same rules');
    const extra = [...abnf].filter((n) => !gbnf.has(n)).sort();
    Assert.deepStrictEqual([...gbnf].filter((n) => !abnf.has(n)), [], 'a rule the ABNF does not name');
    // The page names the ABNF's extras rather than claiming parity for
    // them: the drift this check exists to catch.
    const files = section(docsText('reference-grammar.md'), '## The published files');
    const claimed = Array.from(files.matchAll(/`([A-Za-z][A-Za-z0-9-]*)`(?=,| and)/g), (m) => m[1]).filter((n) => abnf.has(n)).sort();
    Assert.deepStrictEqual(claimed, extra, 'the page must name exactly the rules the ABNF writes out');
});
(0, node_test_1.test)('the-grammar-orderings-are-the-files', () => {
    ...
    for (const long of names) {
        for (const short of names) {
            if (long !== short && long.startsWith(short)) {
                Assert.ok(names.indexOf(long) < names.indexOf(short), long + ' must precede ' + short + ' in the name rule');
                pairs.push([long, short]);
            }
        }
    }
    const scalar = abnfRule('scalar');
    Assert.ok(scalar.indexOf('exact') < scalar.indexOf('number'), 'exact must precede number in the scalar rule');
    const rows = Array.from(section(docsText('reference-grammar.md'), '## Where order matters')
        .matchAll(/^\| `([a-z]+)` \| `([a-z]+)` \|/gm), (m) => [m[1], m[2]]);
    Assert.deepStrictEqual(rows, [['exact', 'number'], ...pairs], 'the ordering table must be every prefix pair the grammar holds');
});
(0, node_test_1.test)('the-tolerated-spellings-mean-what-the-page-says', () => {
    const rows = Array.from(section(docsText('reference-grammar.md'), '## What the parser also accepts')
        .matchAll(/^\| [^|]+ \| `([^`]+)` \| `([^`]+)` \|/gm), (m) => [m[1], m[2]]);
    Assert.ok(4 < rows.length, 'no tolerated spellings read from the page');
    const wrong = rows.filter(([src, canon]) => new aontu_1.Aontu().unify(src).canon !== canon)
        .map(([src, canon]) => src + ' is ' +
        new aontu_1.Aontu().unify(src).canon + ', not ' + canon);
    Assert.deepEqual(wrong, [], 'spellings whose canonical form is not what the page states');
});
(0, node_test_1.test)('the-archive-allowlist-is-the-sources', () => {
    const pkg = Fs.readFileSync(Path.join(REPO, 'ts', 'src', 'pkg.ts'), 'utf8');
    const list = (name) => {
    ...
    const rows = section(docsText('reference-packages.md'), '## What an archive may hold')
        .split('\n')
        .filter((l) => l.startsWith('| ') && l.includes('`'))
        .map((l) => Array.from(l.matchAll(/`([^`]+)`/g), (m) => m[1]));
    Assert.deepStrictEqual(rows, groups, 'the allowlist table must carry the engine groups, in order');
});
