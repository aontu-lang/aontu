...
const hints = {
    ...
    id_ancestor: 'An id() named a value AND SOMETHING INSIDE IT as the same entity.\nIdentity merge unifies every node carrying a name with every other\none carrying it, so naming a node and its own descendant asks for a\nvalue that CONTAINS ITSELF -- not a merge that gives a bad answer,\none that gives no answer at all. Two sibling positions are the case\nidentity exists for; an ancestor and its descendant are not.\n \nExamples:\n  {p:id(x)&{c:1}, q:id(x)&{d:2}} -> {..}  # Siblings merge;\n  a:id(x) & {b:id(y) & {c:1}}    -> {..}  # ... so do distinct names;\n  a:id(x) & {b:id(x)}            -> nil   # ... but not a node inside\n                                          #     itself.',
