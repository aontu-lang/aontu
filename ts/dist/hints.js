...
const hints = {
    ...
    include_extension: 'An @\"...\" include named a file the engine does not read as Aontu\nsource. THE EXTENSION DECIDES: `.aon` and `.aontu` are Aontu source,\nand `.json` and `.jsonld` are read as Aontu too, because JSON is a\nsubset of the grammar. Anything else -- and a name with no extension\n-- is refused rather than guessed at: a guess produces a document\nthat looks right and is not.\n \nExamples:\n  a:@\"model.aon\"    -> {..}  # Aontu source;\n  a:@\"vocab.jsonld\" -> {..}  # ... JSON is a subset, so it parses;\n  a:@\"notes.txt\"    -> nil   # ... but text is not a document;\n  a:@\"data\"         -> nil   # ... and neither is an unnamed kind.',
