...
describe('vet-findings', () => {
  ...
  test('sites-are-role-tagged-data-first', () => {
    const r = vet(SCHEMA, 'service: { name: "auth", port: "8080" }',
      { schemaUrl: 'service.aontu', dataUrl: 'deploy.json' })
...
describe('vet-display-file', () => {
  ...
  test('a-name-with-no-base-to-relativise-against-is-left-alone', () => {
    ...
    Assert.equal(displayFile('entry.aontu', 'entry.aontu', 'x/entry.aontu'),
      'entry.aontu')
    ...
    Assert.equal(displayFile(abs, 'entry.aontu', undefined), abs)
    // ... nor an empty url, nor one that is already relative.
    Assert.equal(displayFile('', 'entry.aontu', 'x/entry.aontu'), '')
    Assert.equal(displayFile('rel.aontu', 'entry.aontu', 'x/entry.aontu'), 'rel.aontu')

    // And the two that DO rewrite, stated here as well because the
    // Go twin states them: bare beside bare, nested through nested.
    Assert.equal(displayFile(abs, 'entry.aontu', entry), 'lib.aontu')
    Assert.equal(
      displayFile(abs, Path.join('a', 'b', 'entry.aontu'), entry),
      Path.join('a', 'b', 'lib.aontu'))
