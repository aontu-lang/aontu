...
The two jobs that carry the Go CLI as a download on every Go release
(docs/release-and-tag.md, "The Go binaries"): `binaries` runs
`go/scripts/binaries.sh` after the tag with `contents: read` and hands
the archives on as an artifact; `release` runs `gh` and nothing else,
with `contents: write`, and puts them on a GitHub Release at
`go/v<VERSION>`. The script, the docs and the changelog entry are
already on the branch; this patch is the part of the same change that
the push could not carry.
