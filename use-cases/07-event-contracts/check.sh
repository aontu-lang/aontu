...
cp "$DIR/expected/orders-v1.canon" "$WORK/served.aontu"
run vsurpcanon 0 -- vet --at '$.registry.order_paid' "$WORK/served.aontu" \
  "$DIR/data/bad/paid-surplus-topic.json"
...
run hashserved 0 -- hash "$WORK/served.aontu"
...
run hda 0 -- hash "$DIR/probes/default-a.aontu"
run hdb 0 -- hash "$DIR/probes/default-b.aontu"
...
# envelope.aontu and reached this path by being CLONED, which used to put
...
# frame used to name envelope.aontu:13 and then quote lines from
# orders-v1.aontu under it -- a real file name over another file's text,
...
# arrow is envelope.aontu's line 13 and the caret sits on the value that
# could not resolve.
run geneval 1 -- "$V1"
has geneval err '[aontu/disjunct_no_gen]'
has geneval err 'envelope.aontu:13:7'
has geneval err 'id: (integer | biginteger) & min(1)'
grep -q 'customer_id' "$DIR/envelope.aontu" \
  && fail "envelope.aontu now holds customer_id; misattribution pin stale" \
  || true
...
run dfillget 0 -- model get '$.Envelope.specversion' "$DIR/envelope.aontu"
...
# envelope.aontu. Pinned structurally, in both directions: the row the
# finding cites holds the time pattern in the file the finding NAMES,
# and the entry file's own line of that number does not.
row="$(grep -o 'envelope.aontu:[0-9]*' "$WORK/btime.out" | head -1 | cut -d: -f2)"
[ -n "$row" ] || fail "btime: schema site does not name envelope.aontu"
sed -n "${row}p" "$DIR/envelope.aontu" | grep -q 'time: re' \
  || fail "btime: envelope.aontu:$row is not the time pattern; site pin stale"
sed -n "${row}p" "$DIR/orders-v1.aontu" | grep -q 'time: re' \
  && fail "btime: orders-v1.aontu:$row holds it too; the pin proves nothing" \
  || true
...
run prefenum 1 -- vet --at '$.E' "$DIR/probes/pref-enum.aontu" \
...
run brkminor 1 -- breaking --against "$V1" "$DIR/orders-v1-1.aontu"
...
run brkat 0 -- breaking --against "$V1" --at '$.Event' "$DIR/orders-v1-1.aontu"
...
run brkmajor 1 -- breaking --against "$V1" "$DIR/orders-v2.aontu"
...
run subpaid 0 -- subsume --at '$.OrderPaid' "$DIR/orders-v1-1.aontu" "$V1"
has subpaid out 'verdict: subsumes'
run subcanc 0 -- subsume --at '$.OrderCancelled' "$DIR/orders-v1-1.aontu" "$V1"
...
# is gone.
run subplaced 0 -- subsume --at '$.OrderPlaced' "$DIR/orders-v1-1.aontu" "$V1"
has subplaced out 'verdict: subsumes'
run subreason 1 -- subsume --at '$.OrderCancelled' "$DIR/orders-v2.aontu" "$V1"
...
run doc 0 -- view doc --depth 2 "$DIR/orders-v1.aontu"
diff -u "$DIR/expected/diagram-doc.txt" "$WORK/doc.out" \
  || fail "the model tree drifted"
run docgate 0 -- view doc --depth 2 \
  --out "$DIR/expected/diagram-doc.txt" --check "$DIR/orders-v1.aontu"
run docsvg 0 -- view doc --depth 2 --as svg \
  --out "$DIR/expected/diagram-doc.svg" --check "$DIR/orders-v1.aontu"
