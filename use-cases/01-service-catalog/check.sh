...
run eval 0 -- "$DIR/system.aontu"
diff -u "$DIR/expected/system.json" "$WORK/eval.out" \
  || fail "system.aontu output drifted from expected/system.json"
ok "system.aontu evaluates to the expected merged catalog"

# 2. Identity survives canonical form.
run canon 0 -- --canon "$DIR/system.aontu"
...
run rel 0 -- relations "$DIR/system.aontu"
...
# the deploy.aontu site that wrote it...
run why1 0 -- model why \
  '$.deploy.regions.eu1.clusters.core.workloads.payments.replicas' \
  "$DIR/system.aontu"
has why1 out 'deploy.aontu:'
ok "why: a workload's replicas is traced to deploy.aontu"
...
run tier1 0 -- model get '$.query.tier1' --keys "$DIR/queries/queries.aontu"
diff -u "$DIR/expected/tier1-keys.txt" "$WORK/tier1.out" \
  || fail "tier1 query drifted"
run exper 0 -- model get '$.query.experimental' --keys "$DIR/queries/queries.aontu"
...
# on the field (bad/wrong-target.aontu spells its own), it flows into
...
run wtgteval 1 -- "$DIR/bad/wrong-target.aontu"
has wtgteval err '[aontu/scalar_value]'
run wtgt 4 -- relations "$DIR/bad/wrong-target.aontu"
...
# path is the answer.
run reach 0 -- reaches '$.catalog.domains.platform.services.gateway' '$.catalog.domains.payments.services.ledger' "$DIR/system.aontu"
...
run badreach 4 -- reaches '$.catalog.domains.platform.services.gateway' '$.catalog.domains.platform.services.nope' "$DIR/system.aontu"
...
run conflict 1 -- "$DIR/bad/tier-conflict.aontu"
...
run kind 1 -- "$DIR/bad/wrong-kind.aontu"
...
run vetok 0 -- vet --at '$.spec.CandidateShape' "$DIR/system.aontu" \
...
run vetbad 1 -- vet --at '$.spec.CandidateShape' --closed \
  "$DIR/system.aontu" "$DIR/data/candidate-malformed.json"
...
run onboard 0 -- "$DIR/proposals/onboard-webhooks.aontu"
run onbrel 0 -- relations "$DIR/proposals/onboard-webhooks.aontu"
has onbrel out 'verdict: pass'
run onbget 0 -- model get '$.catalog.domains.platform.services.webhooks' "$DIR/proposals/onboard-webhooks.aontu"
...
run badref 1 -- "$DIR/proposals/onboard-badref.aontu"
...
run graph 0 -- view graph --relation dependsOn --group-by owner "$DIR/system.aontu"
...
run gate 0 -- view matrix --relation dependsOn --order partition --closure \
  --out "$DIR/expected/diagram-matrix.txt" --check "$DIR/system.aontu"
...
run doc 0 -- view doc --depth 2 "$DIR/system.aontu"
