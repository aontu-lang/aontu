...
APP="$TMP/app"
mkdir -p "$APP"
cp "$DIR/consumer/pkg.aon" "$DIR/consumer/main.aon" "$DIR/consumer/gate.aon" "$APP/"
...
[ ! -e "$APP/aontu_meta/pkg-lock.aon" ] || die "cold tidy must not write a partial lockfile"
...
diff -u "$DIR/consumer/aontu_meta/pkg-lock.aon" "$APP/aontu_meta/pkg-lock.aon" \
  || die "fresh lockfile differs from the committed consumer/aontu_meta/pkg-lock.aon"
...
PIN="$(grep -o 'aon1-[A-Za-z0-9_-]*' "$APP/aontu_meta/pkg-lock.aon")"
ARCHIVE="$(grep -o 'sha256:[0-9a-f]*' "$APP/aontu_meta/pkg-lock.aon")"
...
LOCK_BEFORE="$(cat "$APP/aontu_meta/pkg-lock.aon")"
...
PIN2="$(grep -o 'aon1-[A-Za-z0-9_-]*' "$APP/aontu_meta/pkg-lock.aon")"
...
PIN3="$(grep -o 'aon1-[A-Za-z0-9_-]*' "$APP/aontu_meta/pkg-lock.aon")"
[ "$PIN3" = "$PIN" ] || die "refactor moved the canon pin: $PIN3 != $PIN"
ARCHIVE3="$(grep -o 'sha256:[0-9a-f]*' "$APP/aontu_meta/pkg-lock.aon")"
...
# ----------------------------------------------- D. the inline pin
# Single-file agent-sandbox mode: no pkg.aon, no lockfile, the hash
...
printf 'svc: @"%s#%s"\nsvc: spec: { name: "audit-log", owner: "sec-ops@corp.example" }\n' \
  "$PKG" "$PIN" > "$INLINE/pinned.aon"
run pin-get 0 model get '$.svc.spec.port' "$INLINE/pinned.aon"
[ "$(cat "$TMP/pin-get.out")" = "8080" ] || die "inline-pinned module: wrong port"
ok "inline #aon1- pin: resolves and verifies with no pkg.aon and no lockfile"
...
run bare-local 1 "$INLINE/bare.aon"
...
mkdir -p "$STORE"
cp "$DIR/platform/service/pkg.aon" "$DIR/platform/service/service.aon" "$STORE/"
APP2="$TMP/app2"
mkdir -p "$APP2/aontu_meta"
cp "$DIR/consumer/pkg.aon" "$DIR/consumer/main.aon" "$APP2/"
cp "$DIR/consumer/aontu_meta/pkg-lock.aon" "$APP2/aontu_meta/"
...
[ -f "$APP2/$VENDORED/pkg.aon" ] || die "vendor did not materialise from the cache"
...
APP3="$TMP/app3"
mkdir -p "$APP3"
cp "$DIR/consumer/pkg.aon" "$DIR/consumer/main.aon" "$APP3/"
...
[ -f "$REPO_DIR/advisory/corp.example/schemas/service.aon" ] || die "no advisory"
...
grep -q '"manifest":"sha256:' "$APP4/aontu_meta/pkg-lock.aon" \
  || die "an acquired package carries no manifest pin"
[ -f "$APP4/$VENDORED/aontu_meta/manifest.aon" ] || die "the vendored tree keeps no manifest"
[ -f "$APP4/$VENDORED/aontu_meta/proof.aon" ] || die "the vendored tree keeps no proof"
run eval-net 0 "$APP4/main.aon"
...
grep -q '"1.4.3"' "$APP4/pkg.aon" || die "get did not edit pkg.aon"
...
sed -i 's/version:"1.4.3"/version:"1.4.4"/' "$MOVED/pkg.aon"
...
# resolution tries every enclosing pkg.aon root, not just the nearest
# one, so a dependency vendored beside its dependant is found.
TAPP="$TMP/tapp"
mkdir -p "$TAPP/aontu_meta/vendor/corp.example/schemas"
cp "$DIR/probes/transitive/app/pkg.aon" "$DIR/probes/transitive/app/main.aon" "$TAPP/"
...
FLATPIN="$(grep -o '"corp.example/schemas/service":{"archive":"sha256:[0-9a-f]*","canon":"aon1-[A-Za-z0-9_-]*' \
  "$TAPP/aontu_meta/pkg-lock.aon" | grep -o 'aon1-[A-Za-z0-9_-]*$')"
...
run hash-trans 0 hash "$TAPP/aontu_meta/vendor/corp.example/schemas/service/service.aon"
...
cp "$DIR/probes/transitive/app/pkg.aon" "$DIR/probes/transitive/app/main.aon" "$BAPP/"
...
[ ! -e "$BAPP/aontu_meta/pkg-lock.aon" ] || die "tidy must not write a lockfile it cannot fill"
...
cp "$DIR/platform/service/pkg.aon" "$RAPP/aontu_meta/vendor/corp.example/schemas/service/"
cp "$DIR/probes/nested-ref/service.aon" "$RAPP/aontu_meta/vendor/corp.example/schemas/service/"
cp "$DIR/probes/transitive/app/main.aon" "$RAPP/"
run hash-ref 0 hash "$RAPP/aontu_meta/vendor/corp.example/schemas/service/service.aon"
...
cp "$DIR/consumer/main.aon" "$APP7/"
sed 's/"1.4.2"/"9.9.9"/' "$DIR/consumer/pkg.aon" > "$APP7/pkg.aon"
