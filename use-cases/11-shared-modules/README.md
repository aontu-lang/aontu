...
consumer/                       the checkout repo: pkg.aontu, main.aontu, gate.aontu,
                                plus the committed aontu_meta/pkg-lock.aontu and
...
`consumer/main.aontu` is a consumer repository's entry: it imports the
...
`aontu view doc --depth 3 consumer/main.aontu` draws it, and `check.sh` pins it
...
**3. `aontu sync` then pins.** Exit 0, and `aontu_meta/pkg-lock.aontu` is
...
{"lock":{"corp.example/schemas/service":{"archive":"sha256:5a5d2fb414290dcef45209212e9bf28cd7d1340cd9d8088ed8eed11d1740e2bf","canon":"aon1-zFHnyVa1fA--g8hTx8lUUhaKzzRUNI--2nDheIMsSFs","v":"1.4.2"}}}
```

The `canon` pin equals `aontu hash platform/service/service.aontu` to
...
3. After the platform tree is copied into `aontu_meta/vendor/`, `sync` is
   ...
   byte-identical to the committed `consumer/aontu_meta/pkg-lock.aontu`.
...
6. `aontu main.aontu` matches `expected/consumer.json`: the consumer
...
7. Hermetic: two runs of the same inputs are byte-identical, as JSON
   and as `--canon`.
8. `aontu hash platform/service/service.aontu` equals the lockfile's
   canon pin.
...
11. `pkg verify` against the tampered store checks bytes before
    ...
    and `aontu_meta/pkg-lock.aontu` is byte-identical afterwards:
...
13. `pkg tidy` run after the same tamper reports `verdict: ok` and
    ...
    read like code.
14. Refactor: replacing the vendored `service.aontu` with the two files
    in `probes/refactor/` (an entry delegating to a reordered,
    re-commented `schema.aontu`) gives new bytes and a new file count,
...
15. Inline pin: a single file with
    `@"corp.example/schemas/service#aon1-zFHn…"`, no `pkg.aontu` and
...
29. `publish --yes … --to repo` writes the read-path layout:
    ...
    `@latest`, and `advisory/corp.example/schemas/service.aontu`.
...
34. A consumer whose `pkg.aontu` names that base and the signer
    ...
    `aontu_meta/manifest.aontu` and `aontu_meta/proof.aontu` beside it, and
...
38. `get corp.example/schemas/service@1.4.3` reports
    ...
    fetches it, and edits `pkg.aontu`: a version named explicitly skips
    the cooldown.
...
45. `vet --at spec gate.aontu data/checkout-good.json` is
    `verdict: valid`.
...
48. The flat tree `sync` writes is the layout a nested import reads:
    resolution tries every enclosing `pkg.aontu` root, nearest first, so
...
53. A vendored tree whose own `pkg.aontu` says `1.4.2` does not satisfy a
...
54. `view doc --depth 3 consumer/main.aontu` draws the figure at the head
