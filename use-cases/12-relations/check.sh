...
ok "model.aontu generates: atoms hold, links stay plain strings"
...
run wkeval 1 -- "$DIR/bad/wrong-kind.aontu"
has wkeval err '[aontu/scalar_value]'
run wk 4 -- relations "$DIR/bad/wrong-kind.aontu"
...
run append 0 -- "$DIR/proposals/append.aontu"
diff -u "$DIR/expected/append.json" "$WORK/append.out" \
  || fail "append proposal output drifted"
run apprel 0 -- relations "$DIR/proposals/append.aontu"
...
run reach 0 -- reaches '$.pipeline.jobs.extract' '$.pipeline.jobs.load' --relation feeds "$DIR/model.aontu"
has reach out 'verdict: reaches'
has reach out '$.pipeline.jobs.extract -> $.pipeline.jobs.transform -> $.pipeline.jobs.load'
run noreach 1 -- reaches '$.pipeline.jobs.load' '$.pipeline.jobs.extract' --relation feeds "$DIR/model.aontu"
...
run graph 0 -- view graph "$DIR/model.aontu"
...
run er 0 -- view graph --as er "$DIR/model.aontu"
