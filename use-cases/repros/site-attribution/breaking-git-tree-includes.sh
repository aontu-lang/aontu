...
#           8080 in the included schema.aontu; a v1 doc with port 9090 is
#           now refused)
...
$AONTU breaking --against 'git#HEAD' entry.aontu && echo "exit=0" || echo "exit=$?"

echo "--- control: same state, old version given as a tree copy ---"
mkdir old && git show HEAD:./entry.aontu > old/entry.aontu \
          && git show HEAD:./schema.aontu > old/schema.aontu
$AONTU breaking --against old/entry.aontu entry.aontu || echo "exit=$? (breaking, as it should be)"
