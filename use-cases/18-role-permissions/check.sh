...
# changes.aontu is never touched.
CASE="$WORK/case"
cp -r "$DIR" "$CASE"
...
get_is() {
  local got
  got="$($AONTU model get "$1" "$CASE/system.aontu" 2>/dev/null)"
...
propose() {
  ...
  $AONTU allow --role "$role" "$CASE/roles.aontu" "$change" \
    >"$WORK/$name.out" 2>"$WORK/$name.err" || got=$?
  echo "$got" >"$WORK/$name.gate"
  if [ "$got" -eq 0 ]; then
    got=0
    $AONTU model set "$change" --entry "$CASE/model.aontu" \
      --overlay "$CASE/changes.aontu" >"$WORK/$name.set" 2>&1 || got=$?
...
run roles 0 -- "$DIR/roles.aontu"
diff -u "$DIR/expected/roles.json" "$WORK/roles.out" \
  || fail "roles.aontu output drifted from expected/roles.json"
hasnt roles out '"Role"'
ok "model.aontu and roles.aontu evaluate to their goldens; the vocabularies stay out"

# ------------------------------------------------- the rules of allow
# 2. An allow entry covers itself and everything below it.
run at 0 -- allow --role product "$DIR/roles.aontu" '$.features'
has at out 'verdict: allowed'
has at out '$.features: allowed by $.roles.product.allow.0 ($.features)'
run below 0 -- allow --role product "$DIR/roles.aontu" '$.features.checkout_v2'
has below out '$.features.checkout_v2: allowed by $.roles.product.allow.0 ($.features)'
run deep 0 -- allow --role qa "$DIR/roles.aontu" '$.tests.smoke.timeout'
...
run above 1 -- allow --role product "$DIR/roles.aontu" '$.services'
has above out 'verdict: refused'
has above out '$.services: refused (no allow entry of product covers it)'
run root 1 -- allow --role qa "$DIR/roles.aontu" '$'
has root out '$: refused (no allow entry of qa covers it)'
run admin 0 -- allow --role admin "$DIR/roles.aontu" '$' '$.services.auth.tier'
...
# keys down.
run star 0 -- allow --role dev "$DIR/roles.aontu" \
...
run starabove 1 -- allow --role dev "$DIR/roles.aontu" '$.deploy.eu1' '$.deploy'
...
run startwo 1 -- allow --role dev "$DIR/roles.aontu" '$.deploy.eu1.canary.replicas.search'
...
# model alone and never opens model.aontu, so a path the model does not
# have (the tier has no `level`) is answered the same way.
run denyat 1 -- allow --role dev "$DIR/roles.aontu" '$.services.auth.tier'
has denyat out '$.services.auth.tier: refused by $.roles.dev.deny.0 ($.services.*.tier)'
run denyabove 1 -- allow --role dev "$DIR/roles.aontu" '$.services.auth' '$.services' '$'
...
run denybelow 1 -- allow --role dev "$DIR/roles.aontu" '$.services.auth.tier.level'
has denybelow out '$.services.auth.tier.level: refused by $.roles.dev.deny.0 ($.services.*.tier)'
run denyowner 1 -- allow --role dev "$DIR/roles.aontu" '$.services.billing.owner'
has denyowner out '$.services.billing.owner: refused by $.roles.dev.deny.1 ($.services.*.owner)'
run beside 0 -- allow --role dev "$DIR/roles.aontu" '$.services.auth.replicas'
...
run order 1 -- allow --role r "$WORK/order.aontu" '$.a.b' '$.a.c'
...
# generates for it.
run uncovered 1 -- allow --role product "$DIR/roles.aontu" '$.services.auth.replicas'
has uncovered out '$.services.auth.replicas: refused (no allow entry of product covers it)'
printf 'roles: r: {}\n' > "$WORK/noallow.aontu"
run noallow 1 -- allow --role r "$WORK/noallow.aontu" '$.a'
...
# is close.
run ops 1 -- allow --role ops "$DIR/roles.aontu" '$.features.dark_mode'
...
run near 1 -- allow --role deve "$DIR/roles.aontu" '$.services'
...
run assign 0 -- allow --role product "$DIR/roles.aontu" \
...
run smuggle 2 -- allow --role product "$DIR/roles.aontu" \
...
run devtext 1 -- allow --role dev "$DIR/roles.aontu" '$.services.auth.replicas' \
...
run qajson 1 -- allow --role qa --format json "$DIR/roles.aontu" '$.tests.smoke' '$.services'
...
run at2 0 -- allow --role release --at '$.policy.roles' "$DIR/policy.aontu" '$.deploy.eu1'
has at2 out '$.deploy.eu1: allowed by $.policy.roles.release.allow.0 ($.deploy)'
run at3 1 -- allow --role dev "$DIR/policy.aontu" '$.services.auth.replicas'
...
grep -qF '"services": "auth": "replicas": 5' "$CASE/changes.aontu" \
  || fail "the allowed change did not reach the overlay"
get_is '$.services.auth.replicas' '5'
run whyval 0 -- model why '$.services.auth.replicas' "$CASE/system.aontu"
has whyval out '$.services.auth.replicas = 5'
has whyval out 'changes.aontu:2:33'
has whyval out '*3'
has whyval out '(pref)'
...
grep -q 'tier' "$CASE/changes.aontu" && fail "a refused change reached the overlay" || true
get_is '$.services.auth.tier' '"critical"'
run dry 0 -- model set '$.services.auth.tier="standard"' --entry "$CASE/model.aontu" \
  --overlay "$CASE/changes.aontu" --dry-run
...
grep -q 'colour' "$CASE/changes.aontu" && fail "the undeclared key reached the overlay" || true
grep -q '"auth": 0' "$CASE/changes.aontu" && fail "the zero count reached the overlay" || true
...
run system 0 -- "$CASE/system.aontu"
diff -u "$DIR/expected/system.json" "$WORK/system.out" \
  || fail "the served view after the loop drifted from expected/system.json"
[ "$(grep -c '^"' "$CASE/changes.aontu")" -eq 4 ] \
  || { cat "$CASE/changes.aontu" >&2; fail "the overlay does not hold exactly four lines"; }
...
run badlist 4 -- allow --include-root "$DIR" --role dev \
  "$DIR/proposals/role-allow-string.aontu" '$.services.auth.replicas'
...
run badrole 4 -- allow --include-root "$DIR" --role dev \
  "$DIR/proposals/add-undeclared-role.aontu" '$.services.auth.replicas'
...
run why 0 -- model why '$.roles.dev.deny.0' "$DIR/roles.aontu"
