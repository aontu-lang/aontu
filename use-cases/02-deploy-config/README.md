...
| File | Layer / role |
|---|---|
| `org-policy.aontu` | org: workload shape (closed), org-rank `***` defaults |
| `team-defaults.aontu` | team: `**` defaults, release train, on-call |
| `fleet.aontu` | service catalog: the source `pack()` iterates |
| `envs/{dev,staging,prod}.aontu` | overlays: `*` defaults via `&:` spreads + concrete pins |
| `stack.aontu` | entry: includes, `pack()` per env, `envguard`, `filter()` alerts, rollout arithmetic |
| `guardrails.aontu` | org bounds, enforced by `vet` over the **built** output |
| `request-schema.aontu` | gate for agent-emitted change candidates (`data/*.json`) |
| `probes/*.aontu` | one file per behaviour below, each pinned by `check.sh` |

Probes that reach `@"../stack.aontu"` resolve an include outside their
own entry root; the CLI warns once per such include, `--trust system`
keeps it, and `--include-root` confines it.

## The model tree

`stack.aontu` is the entry: four layers of authority unified into one
...
`aontu view doc --depth 2 stack.aontu` draws it, and `check.sh` pins it
...
- **Ranked preferences are the layering mechanism.** Generation picks
  ...
  a concrete value beats them all (`probes/rank-ladder.aontu`). So the
  ...
  Four layers, six files, one `aontu stack.aontu`, and every value lands
...
- **`close()` on the workload shape** (applied inside the pack
  ...
  Cannot add to closed structure. ...
    --> probes/typo-overlay.aontu:7:41
  ```
  ...
  (`probes/close-pack-absorb.aontu`); the overlay lands on the generated
  child:
...
- **Conflicts are located and attributable.** Two files pinning
  ...
  line and column (`probes/conflict.aontu`). A layer cannot remove a
  lower layer's key either: unification only adds information, so
  `null` over a boolean is `[aontu/scalar_kind]`
  (`probes/remove-key.aontu`).

- **`why` is multi-layer attribution**, at plain paths and through
  the generator alike:

  ```
  $ aontu model why '$.defs.workload.logLevel' stack.aontu
  $.defs.workload.logLevel = **"debug"|***"info"|string
    1. ***"info"|string  .../org-policy.aontu:31:15
    2. **"debug"|string  .../team-defaults.aontu:12:28

  $ aontu model why '$.deploy.dev.workloads.web.logLevel' stack.aontu
  $.deploy.dev.workloads.web.logLevel = **"debug"|***"info"|string
    1. **"debug"|***"info"|string  .../team-defaults.aontu:12:28

  $ aontu model why '$.deploy.prod.workloads.billing.replicas' stack.aontu
  $.deploy.prod.workloads.billing.replicas = 12
    1. *4|integer  .../envs/prod.aontu:12:15  (spread)
    2. 12  .../envs/prod.aontu:18:22
    3. ***2|integer  .../org-policy.aontu:30:15
  ```
...
- **Constraint atoms live in `guardrails.aontu` and
  `request-schema.aontu` and are enforced with `vet`** over the built
  ...
  $.replicas: constraint [conflict]
    ...
    schema: request-schema.aontu:17:23 (integer&min(1)&max(24))
  ```
...
- **Projection over the catalog.** `unique(port)` states "every
  ...
  (`probes/unique-port.aontu`: two services sharing 8080 are refused
...
- **Derived arithmetic lives in `rollout:`**, a plain map outside the
  ...
  `[aontu/mapval_no_gen]` (`probes/surge-from-default.aontu`). `*` is
  the preference marker, not multiplication: `$.replicas * 2` is a
  parse error (`[aontu/unexpected]`, `probes/multiply.aontu`), and
...
5. `why` on `$.defs.workload.logLevel` names both `org-policy.aontu` and
   `team-defaults.aontu` with their ranks and lines; on
   `$.deploy.prod.workloads.billing.replicas` it names `envs/prod.aontu`
   and the pin 12; on the generated `$.deploy.dev.workloads.web.logLevel`
   it names `team-defaults.aontu` and the winning `debug`.
6. `vet guardrails.aontu expected/stack.json` is `verdict: valid`.
...
```sh
aontu stack.aontu > built.json              # build the deployment document
aontu vet guardrails.aontu built.json       # org bounds over the built output
aontu vet request-schema.aontu data/rollout-good.json   # gate an agent candidate
```
