...
get_is() { # path expected label
  local got
  got="$(aontu model get "$1" "$DIR/main.aontu")"
...
ENV_JSON="$(aontu model get '$.deploy.billing.spec.template.spec.containers.0.env' "$DIR/main.aontu")"
...
# ------------------------------------------------------------ guardrails
run vet-good 0 vet "$DIR/guardrails.aontu" "$DIR/expected/manifests.json"
has vet-good 'verdict: valid' "guardrail verdict"
ok "vet guardrails over rendered manifests: valid"

run vet-tampered 1 vet "$DIR/guardrails.aontu" "$DIR/data/manifests-tampered.json"
...
run vet-unknown 1 vet --closed "$DIR/guardrails.aontu" "$DIR/data/manifests-unknown-key.json"
...
run onboard-good 0 vet "$DIR/request-schema.aontu" "$DIR/data/onboard-good.json"
has onboard-good 'verdict: valid' "good candidate"
ok "agent onboarding candidate within policy: valid"

run onboard-bad 1 vet "$DIR/request-schema.aontu" "$DIR/data/onboard-bad.json"
...
run onboard-json 1 vet --format json "$DIR/request-schema.aontu" "$DIR/data/onboard-bad.json"
...
run p-default-with-bounds 0 "$DIR/probes/default-with-bounds.aontu"
...
run p-hole-member-access 0 "$DIR/probes/hole-member-access.aontu"
...
$AONTU view doc --depth 2 "$DIR/main.aontu" > "$TMP/doc.out" 2>/dev/null \
...
run docgate 0 view doc --depth 2 \
  --out "$DIR/expected/diagram-doc.txt" --check "$DIR/main.aontu"
run docsvg 0 view doc --depth 2 --as svg \
  --out "$DIR/expected/diagram-doc.svg" --check "$DIR/main.aontu"
