...
2. **Seed-data generator.** Evaluating `seed.aontu` *is* generating the
...
3. **Code source.** `xf-domain.aontu` and `xf-order.aontu` walk the record
...
`seed.aontu` layers the deterministic seed ledger onto the domain model,
...
`aontu view doc --depth 2 seed.aontu` draws it, and `check.sh` pins it
...
- `seed.aontu`: includes the domain; an exact price book in `0d`
...
- `exact-money.aontu`: the money schema stated on the exact leaf
  (`amountEur: bigdecimal & min(0d0)`). Only a `0d` literal produces
  a `bigdecimal`, and JSON has no such spelling, so an `.aontu` record
  satisfies this schema and a strictly JSON record cannot.
- `money-wire.aontu` (the money schema for a JSON wire: a decimal
  string with a fixed scale, an ISO 4217 currency beside it, and an
  optional-but-constant conversion mark (`dec?: "bigdecimal:2"`)
  naming the leaf and the scale. `money-convert.aontu` writes the
...
- `xf-domain.aontu`: the schema as code: `pack(type($.schema), …)` walks
...
- `xf-order.aontu`: one model, two targets: the same walk with the two
  ...
  see it.
- `reporting.aontu`: a wider projection; `subsume` proves it sound.
- `gaps/`: one-file models, each pinning a single behaviour of the
  arithmetic and constraint families; see below.

Money on **records** in `domain.aontu` is integer minor units (cents),
one of the two supported spellings; the other is the decimal-string
wire form in `money-wire.aontu`. Exact arithmetic lives in the price
...
`gaps/float-mix.aontu` adds a `0d` book price to a float and is refused
...
`ledgerId: integer` refuses that record (`bad/id-trap-schema.aontu`
against `data/customer-bigid.aontu`):
...
`number` would admit floats, so `domain.aontu` spells the field as the
...
same way: `reporting.aontu` leaves `ledgerId` out of its Customer view,
and `bad/reporting-int64.aontu`, which declares it `integer`, fails
subsumption.

A link is a tree path (`customerId: path("$.customers.cust-1001")`),
so `domain.aontu` pins the address shape in the type
...
`reporting.aontu` restates the domain's `*"open"` default on `status`
...
- `agg-sum.aontu` derives an invoice total from its lines:
...
- `multiply.aontu` computes `mul(2, 1999)` and integer-cent VAT as
  ...
  `star-token.aontu` shows `2 * 1999` refused at parse with
  `[aontu/unexpected]`.
- `unique-by-field.aontu` puts `unique(ledgerId)` on the customer bag:
...
- `spread-cross-field.aontu` states `gross = net + tax` once, as a
...
- `include-alias-spread/` is a two-file model: `vocab.aontu` declares a
  named alias (`T: type(integer & min(1))`) and a record type that
  references it, and `main.aontu` includes it with `@"./vocab.aontu"` and
...
1. `aontu seed.aontu` matches `expected/seed.json` byte for byte:
   defaults filled (`ord-7002.status` is `"open"`), every `id` equal
   to its key, and one receivables account per customer.
2. `aontu model get '$.pricing.bundles' --canon seed.aontu` matches
...
3. `$.reconcile.exactPath` is `0d0.3` in canon and `0.3` generated:
   `0d0.1 + 0d0.2` is exactly `0d0.3`, and the pin in `seed.aontu`
   holds.
4. One `vet` command over `seed.aontu` and three data files (two
   ...
   verdict stays `valid`.
5. `bad/order-dangling.aontu`, an order whose `customerId` names
...
8. `bad/customer-id-lossy.json`, plain JSON carrying
   ...
   id.
9. `bad/id-trap-schema.aontu`, which declares `ledgerId: integer &
   min(1)`, refuses `data/customer-bigid.aontu` with
   ...
   `0d9007199254740993`.
10. `exact-money.aontu` accepts `data/quote-exact.aontu` (`0d10.50`) and
    refuses `data/quote-float.json` (`10.5`) with `[aontu/constraint]`
    against `bigdecimal`:

    ```
    $ aontu vet exact-money.aontu data/quote-float.json
    verdict: invalid
...
11. `data/quote-0d.json`, which spells the amount `0d10.50`, vets as
    ...
    halves are asserted.
12. `aontu vet --at '$.schema.Customer' domain.aontu
    data/customer-record.json` validates one bare record against one
    named type: `verdict: valid`.
13. `aontu subsume reporting.aontu domain.aontu` is `verdict: subsumes`,
    exit 0: every document the domain admits, the reporting view
    admits.
14. `aontu subsume bad/reporting-int64.aontu domain.aontu` is
    `undecided`, exit 3, citing `sub_disjunct_distribution` and the
    domain's `biginteger&min(1)` alternative that the view's `integer`
    does not admit.
15. `gaps/agg-sum.aontu` evaluates with `"total": 4008` and
    `"largest": 3998`.
16. `gaps/multiply.aontu` evaluates with `"amount": 3998` and
    `"vatCents": 759`.
17. `gaps/star-token.aontu` is refused at parse with
    `[aontu/unexpected]`, exit 1.
18. `gaps/float-mix.aontu` is refused with `[aontu/exact_float_mix]`,
    exit 1.
19. `gaps/spread-cross-field.aontu` is refused with `[aontu/no_path]`,
    exit 1.
20. `gaps/list-length-template.aontu` is refused with
    `[aontu/constraint]`, exit 1.
21. `gaps/unique-by-field.aontu` is refused with `[aontu/constraint]` at
    `$.customers`, exit 1.
22. `gaps/include-alias-spread/main.aontu` generates the record through
    the include, `"ledgerId": 5` and `"id": "cust-1001"`, exit 0.
23. `data/quote-wire.json` is strict JSON (`JSON.parse` accepts it),
    and `aontu vet money-wire.aontu data/quote-wire.json` is
    `verdict: valid`.
...
26. `aontu jsonschema --at '$.Money' money-wire.aontu` carries the
...
27. `aontu --canon money-convert.aontu` prints `"amount":0d3998.19`,
...
28. The two transforms write their goldens byte for byte through
    jostraca: `xf-domain.aontu` answers `expected/render/domain.ts`, one
    exported interface per record, and `xf-order.aontu` answers
...
```sh
aontu seed.aontu                                                                            # generate the fixture set
aontu vet seed.aontu data/order-batch-1.aontu data/order-batch-2.aontu data/customer-bigid.aontu  # vet a batch
aontu model get out xf-order.aontu                                                                 # the schema as TypeScript and Go
```
