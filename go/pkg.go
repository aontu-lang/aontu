...
const lockHeader = "# pkg-lock.aontu (generated by `aontu sync`; do not edit)\n"
...
func packageSelf(dir string, opts *PkgOptions) PackageSelf {
	self := PackageSelf{Main: "main.aontu", Publish: "private", Retract: []string{}}
...
func storedManifestOf(dir string) *storedManifest {
	data, err := os.ReadFile(filepath.Join(dir, metaDir, "manifest.aontu"))
	if nil != err {
		return nil
	}
...
func ManifestText(m *PkgManifest, extra map[string]any, opts *PkgOptions) string {
	...
	return evalPkg(string(src), "manifest.aontu", opts).canon
}
