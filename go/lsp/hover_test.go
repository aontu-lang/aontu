...
func TestCompletionsList(t *testing.T) {
	...
	if len(c) != 52 {
		t.Fatalf("expected 52 completions, got %d", len(c))
	}
...
func TestBuiltinFuncNamesParity(t *testing.T) {
	...
	want := []string{"above", "acyclic", "add", "below", "close", "copy", "deprecate", "div", "each", "filter", "greatest", "hide", "inverse", "key", "least", "length", "lower", "match", "max", "min", "mod", "move", "mul", "must", "neq", "open", "pack", "path", "pick", "pref", "re", "refer", "rel", "rem", "sub", "sum", "super", "type", "unique", "upper"}
	if len(got) != len(want) {
...
func TestHandlerCompletion(t *testing.T) {
	...
	if len(items) != 52 {
		t.Errorf("expected 52 completion items, got %d", len(items))
	}
}
