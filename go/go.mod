module github.com/aontu-lang/aontu/go

go 1.24.7

require (
	github.com/jostraca/jostraca/go v0.38.0
	github.com/tabnas/abnf/go v0.4.15
	github.com/tabnas/expr/go v0.5.10
	github.com/tabnas/ini/go v0.5.9
	github.com/tabnas/json/go v0.5.10
	github.com/tabnas/json5/go v0.5.8
	github.com/tabnas/jsonc/go v0.5.7
	github.com/tabnas/jsonic/go v0.7.1
	github.com/tabnas/multisource/go v0.5.8
	github.com/tabnas/parser/go v0.12.2
	github.com/tabnas/path/go v0.3.8
	github.com/tabnas/toml/go v0.5.8
	github.com/tabnas/yaml/go v0.5.8
	golang.org/x/mod v0.32.0
	golang.org/x/text v0.34.0
)

require (
	github.com/rjrodger/shape/go v0.5.3 // indirect
	github.com/tabnas/bnf/go v0.1.19 // indirect
	github.com/tabnas/directive/go v0.5.8 // indirect
	github.com/tabnas/hoover/go v0.3.9 // indirect
)
